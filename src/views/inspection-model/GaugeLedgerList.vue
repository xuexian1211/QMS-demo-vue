<template>
  <div class="page-container">
            ...
            <a-form layout="inline" :model="ledgerQuery">
              ...
              <a-form-item label="状态">
                <a-select v-model:value="ledgerQuery.status" style="width: 120px" allow-clear placeholder="请选择">
                  <a-select-option value="In Use">在用</a-select-option>
    ...
    <!-- Ledger Edit Modal -->
    <a-modal v-model:visible="ledgerModalVisible" :title="isEditLedger ? '编辑量检具台账' : '新增量检具台账'" width="600px"
      @ok="saveLedger">
...
<script setup lang="ts">
  ...
  const ledgerColumns = [
    ...
    { title: '量具名称', dataIndex: 'gaugeName', key: 'gaugeName', width: 150 },
    { title: '规格型号', dataIndex: 'model', key: 'model', width: 120 },
    { title: '状态', dataIndex: 'status', key: 'status', width: 100 },
    { title: '下次校准', dataIndex: 'nextCalDate', key: 'nextCalDate', width: 120 },
  ...
  const loadLedgers = (typeId: number) => {
    ...
    setTimeout(() => {
      ...
      if (typeId === 301 || typeId === 3011) {
        data = [
          { id: 1001, gaugeNo: 'G-001', gaugeName: '数显卡尺01', model: 'M-150', status: 'In Use', nextCalDate: '2023-12-31', description: '产线A使用' },
          { id: 1002, gaugeNo: 'G-002', gaugeName: '数显卡尺02', model: 'M-150', status: 'Calibrating', nextCalDate: '2023-10-01', description: '送检中' }
        ]
      }
  ...
  const handleAddLedger = () => {
    ...
    Object.assign(ledgerForm, { id: null, gaugeNo: '', gaugeName: selectedType.value?.typeName || '', model: '', status: 'In Use', nextCalDate: null, description: '' })
    ledgerModalVisible.value = true
  }

  const handleEditLedger = (record: any) => {
    isEditLedger.value = true
    // creating a copy and handling date conversion
    const formData = { ...record }
    if (formData.nextCalDate) {
      formData.nextCalDate = dayjs(formData.nextCalDate)
    }
  ...
  const saveLedger = () => {
    ...
    if (saveData.nextCalDate) {
      saveData.nextCalDate = dayjs(saveData.nextCalDate).format('YYYY-MM-DD')
    }
