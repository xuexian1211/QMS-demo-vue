<template>
  <div class="scheme-edit-page">
    ...
    <!-- 集团方案只读提示 -->
    <a-alert v-if="isReadOnly" type="warning" show-icon message="集团级方案对工厂用户只读，如需修改请联系集团管理员，或基于此方案创建一个新的本地方案。"
        ...
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item label="方案编码" name="schemeCode">
              <a-input v-model:value="form.schemeCode" :disabled="isView || isEdit || isReadOnly"
                placeholder="如: SCH-IQC-001" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
        ...
        <a-row :gutter="24">
          ...
          <a-col :span="8">
            <a-form-item label="引用模板">
              <a-select v-model:value="selectedTemplateId" :disabled="isView || isReadOnly || details.length > 0"
                placeholder="选择模板可自动带出检验项目" allow-clear @change="handleTemplateChange">
    ...
    <a-card class="tab-card">
      <a-tabs v-model:activeKey="activeTab">
            ...
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-if="column.key === 'inspItemName'">
                <template v-if="!isView && record._editing">
                  <a-select v-model:value="record.inspItemId" size="small" style="width:160px"
              ...
              <template v-if="column.key === 'samplingRuleName'">
                <template v-if="!isView && record._editing">
                  <a-select v-model:value="record.samplingRuleCode" size="small" style="width:160px"
                    ...
                    <a-select-option v-for="r in samplingRules" :key="r.code" :value="r.code">{{ r.name
                      }}</a-select-option>
                  </a-select>
                </template>
        ...
        <a-tab-pane key="strategies">
          ...
          <a-table :columns="strategyColumns" :data-source="strategies" row-key="id" size="small" :pagination="false">
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'matchSummary'">
                <a-space wrap>
                  ...
                  <a-tag
                    v-if="!record.matchDimension?.materialName && !record.matchDimension?.supplierName && !record.matchDimension?.customerName && !record.matchDimension?.operationName"
                    color="default">
                    通用（兜底）
                  </a-tag>
                </a-space>
        ...
        <a-tab-pane key="phenomenon" tab="不良现象防错配置">
          ...
          <a-table :columns="phenomenonColumns" :data-source="phenomenonMaps" row-key="id" size="small"
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'action'">
                <a-button type="link" danger size="small" @click="handleRemovePhenomenon(record)"
                  v-if="!isView && !isReadOnly">删除</a-button>
              </template>
            </template>
          </a-table>
        </a-tab-pane>
    ...
    <!-- 策略编辑弹窗 -->
    <a-modal v-model:visible="showStrategyModal" :title="editingStrategy?.id?.startsWith('new') ? '新增绑定策略' : '编辑绑定策略'"
      width="680px" @ok="saveStrategy" ok-text="确定" cancel-text="取消">
      ...
      <a-form v-if="editingStrategy" :model="editingStrategy" :label-col="{span:6}" :wrapper-col="{span:16}">
        ...
        <a-form-item label="客户" v-if="editingStrategy.contextType === 'OQC'">
          <a-select v-model:value="editingStrategy.matchDimension.customerId" allow-clear placeholder="不限客户"
            ...
            <a-select-option v-for="c in customerOptions" :key="c.id" :value="c.id">{{ c.code }} - {{ c.name
              }}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="工序" v-if="editingStrategy.contextType === 'IPQC'">
          <a-select v-model:value="editingStrategy.matchDimension.operationNo" allow-clear placeholder="不限工序"
            ...
            <a-select-option v-for="o in operationOptions" :key="o.no" :value="o.no">{{ o.no }} {{ o.name
              }}</a-select-option>
          </a-select>
        </a-form-item>
    ...
    <a-modal v-model:visible="showMatchSim" title="策略匹配模拟器" width="700px" @ok="runMatchSim" ok-text="开始模拟">
      ...
      <a-form :model="simContext" :label-col="{span:6}" :wrapper-col="{span:16}">
        ...
        <a-form-item label="物料">
          <a-select v-model:value="simContext.materialId" allow-clear placeholder="请选择">
            <a-select-option v-for="m in materialOptions" :key="m.id" :value="m.id">{{ m.code }} - {{ m.name
              }}</a-select-option>
          </a-select>
        </a-form-item>
      ...
      <div v-if="simResult" class="sim-result">
        <a-result :status="simResult.matched ? 'success' : 'warning'"
          ...
          <template #subTitle>
            <div v-if="simResult.matched">
              <a-descriptions bordered size="small" :column="2">
                ...
                <a-descriptions-item label="触发条件">{{ triggerConditionText(simResult.strategy?.triggerCondition)
                  }}</a-descriptions-item>
              </a-descriptions>
            </div>
...
<script setup lang="ts">
  ...
  // ─── 保存 & 返回 ──────────────────────────
  const handleBack = () => router.push('/inspection-model/insp-schemes')
