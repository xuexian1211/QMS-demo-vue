  ...
  <div class="page-container">
    ...
    <a-card title="搜索条件" size="small" class="search-card">
      <a-form layout="inline" :model="query">
        <a-form-item label="方法名称"><a-input v-model:value="query.methodName" style="width:220px" /></a-form-item>
        <a-form-item label="附件ID"><a-input v-model:value="query.attachmentId" style="width:220px" /></a-form-item>
        <a-form-item>
...
const data = ref<any[]>([])
const query = reactive({ methodName: '', attachmentId: '' })

const columns = [
  { title: '方法名称', dataIndex: 'methodName', key: 'methodName' },
  { title: '操作步骤', dataIndex: 'operationText', key: 'operationText', width: 380, customRender: ({ text }: any) => text ? String(text).slice(0, 50) + (String(text).length > 50 ? '…' : '') : '' },
  ...
  { title: '操作', key: 'actions', customRender: ({ record }: any) => {
    ...
    return h(ASpace, null, { default: () => [
      h(AButton, { size: 'small', type: 'link', onClick: () => router.push(`/inspection-model/insp-methods/edit/${record.id}`) }, { default: () => '编辑' }),
      h(AButton, { size: 'small', type: 'link', onClick: () => router.push(`/inspection-model/insp-methods/view/${record.id}`) }, { default: () => '查看' }),
    ] }) } },
]
...
const load = () => {
  ...
  let res = all
  if (query.methodName) res = res.filter(r => r.methodName.includes(query.methodName))
  if (query.attachmentId) res = res.filter(r => (r.attachmentId||'').includes(query.attachmentId))
  pagination.total = res.length
  data.value = res
}

const search = () => { pagination.current = 1; load() }
const reset = () => { Object.assign(query, { methodName: '', attachmentId: '' }); search() }
const toCreate = () => router.push('/inspection-model/insp-methods/create')
const toEdit = () => { if (selectedRows.value[0]) router.push(`/inspection-model/insp-methods/edit/${selectedRows.value[0].id}`) }
...
const exportCsv = () => {
  const header = ['方法名称','操作步骤','附件ID']
  const rows = data.value.map(r => [r.methodName, String(r.operationText).replace(/\n/g,' '), r.attachmentId||''])
