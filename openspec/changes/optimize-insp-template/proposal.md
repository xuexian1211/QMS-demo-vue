...
- **多组织支持 (Multi-Org)**：所有核心实体（模板、方案、策略、规格）均强制绑定 `orgId`，确保物理工厂间的数据隔离与逻辑独立。
- **UI/UE 布局重构**：参考检验模板布局，采用“上方基础信息 + 下方多页签详情”模式。页签包含：检验项目明细、明细不良现象绑定、策略绑定、物料检验规格。
- **策略维度深度扩展**：在现有维度基础上，新增物料组（Material Group）、客户（Customer）等关键维度，并支持 IPQC 类型（首件/巡检/末件）的细粒度绑定。
- **物料检验规格 (MaterialSpec) 智能绑定与同步**：实现方案与物料规格的自动联动。支持从物料档案自动获取规格，并允许在方案中编辑后反向同步至物料档案。
