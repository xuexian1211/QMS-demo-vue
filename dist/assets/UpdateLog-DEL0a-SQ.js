var On=Object.defineProperty;var fn=(r,n,e)=>n in r?On(r,n,{enumerable:!0,configurable:!0,writable:!0,value:e}):r[n]=e;var C=(r,n,e)=>fn(r,typeof n!="symbol"?n+"":n,e);import{d as vn,r as on,m as kn,k as cn,c as v,e as b,p as bn,s as ln,F as Vn,t as dn,x as Qn,o as N}from"./index-DD--uQAh.js";import{_ as Un}from"./_plugin-vue_export-helper-DlAUqK2U.js";const xn=`# 舜富 QMS 系统菜单重构方案\r
\r
## 1. 重构背景与设计思路\r
\r
...
`,jn=`# 物料分类管理 (Material Category) - 详细设计文档\r
\r
## 1. 模块概述\r
**模块名称**：物料分类管理\r
**所属子系统**：基础数据管理模块\r
...
`,Wn=`# 生产班组档案 (Production Team) - 详细设计文档\r
\r
## 1. 模块概述\r
**模块名称**：生产班组管理\r
...
`,Xn=`基于需求方案及前期设计的检验模型，**实验室送检（Laboratory Testing）**模块作为高精度、专项理化分析的核心，其设计旨在确保原材料及产品符合特定技术标准，并提供自动化的数据存证 。\r
...
3. 处置：实验员定期清理样品，并在系统中勾选“已销毁”或“退还”，确保台账实物一致。`,$n=`# IQC 来料检验执行与 MRB 进料不良异常处理 — 详细设计文档\r
\r
> **版本**: V2.0  \r
> **日期**: 2026-03-02  \r
...
`,re=`# 变化点管理（Change Point Management）详细设计文档
...
| V2.0 | 2026-03-06 | 补充风险分析、验证计划、优化建议，基于已实现代码完善文档 | AI助手 |
...
| 端 | 功能 | 实现状态 |
|---|---|:---:|
| **PC 端（核心）** | 变化点台账与追溯列表、申报/编辑/查看详情页、试生产验证任务中心、变化点中央看板、风险矩阵规则配置 | ✅ 已实现 |
| **PDA/移动端（辅助）** | 现场一键申报页（4M1E 大图标分类）、现场验证录入、班组长快速审批 | ⏳ 待实现 |
| **系统集成** | MES 互锁 API（锁机/解锁）、钉钉通知推送 | ⏳ 前端已模拟，后端待对接 |
...
- **状态管理：** 组件内部 reactive/ref（未来可迁移至 Pinia）
- **路由：** Vue Router 4.2+
...
src/
├── types/
│   └── change-point.ts                    # 完整类型定义与常量配置
├── views/production-quality/change-point/
│   ├── ChangePointList.vue                # 台账列表与审批
│   ├── ChangePointEdit.vue                # 申报/编辑/查看详情页
│   ├── VerificationCenter.vue             # 试生产验证任务中心
│   ├── ChangePointDashboard.vue           # 中央看板
│   └── RiskMatrixConfig.vue               # 风险矩阵规则配置
└── router/index.ts                        # 已添加路由配置
\`\`\`
...
### 5.1 变化点台账列表页 (\`ChangePointList.vue\`)

**核心功能：**
- ✅ 状态快速切换 Tab（全部/待审批/审批中/验证中/已放行/已闭环）
- ✅ 多维度搜索筛选（单号/标题、变化类型、风险等级、提报时间）
- ✅ 数据表格展示（含颜色编码的状态徽章、风险标签）
- ✅ 操作按钮（查看、编辑、审批弹窗）
- ✅ 分页与排序
...
const statusTabs = computed(() => [
  { key: 'all', label: '全部', value: '', count: allCount.value },
  { key: 'pending', label: '待审批', value: 'submitted,approving', count: pendingCount.value },
  // ...
])

// 审批弹窗逻辑
const handleApprove = (record: ChangePointRecord) => {
...
**UI 亮点：**
- 使用状态徽章（Badge）和颜色标签清晰展示状态
- 鼎级操作按钮布局，符合用户操作习惯
- 响应式设计，支持宽屏和窄屏

### 5.2 变化点申报/编辑页 (\`ChangePointEdit.vue\`)
...
\`\`\`typescript
// 4M1E 类型选择器
const typeSelector = ref<ChangePointType>('machine')
...
const submitApproval = () => {
  if (approveForm.result === 'approved') {
    if (['medium', 'high'].includes(formData.riskLevel)) {
      ...
      message.success('审批通过！系统已自动调用 MES 锁机接口。')
    } else {
      formData.status = 'released'
      message.success('审批通过！低风险变化点已直接放行。')
    }
  }
}
\`\`\`
...
**UI 亮点：**
- 左右分栏布局：左侧表单主区域，右侧状态追踪
- 4M1E 图标选择器提供直观的分类体验
- 风险等级使用颜色编码和图标强化视觉层次
- MES 锁机横幅（Alert）清晰提示当前状态

### 5.3 试生产验证任务中心 (\`VerificationCenter.vue\`)

**核心功能：**
- ✅ 验证任务列表（含进度条、截止时间、超时标记）
- ✅ 统计摘要卡片（待开始/验证中/已超时/已放行）
- ✅ 任务详情视图（全屏替换列表）
- ✅ 单件验证结果录入弹窗
- ✅ 合格计数归零逻辑（出现不合格时）
- ✅ 放行确认操作（调用 MES 解锁）
...
\`\`\`typescript
// 计算验证进度百分比
const calcPercent = (record: TaskRecord) => 
  Math.round(record.passedCount / record.requiredCount * 100)

// 进度条状态
const progressStatus = (record: TaskRecord) => {
  if (record.status === 'passed') return 'success'
  if (record.taskItems?.some(i => i.result === 'fail')) return 'exception'
  if (record.status === 'running') return 'active'
  return 'normal'
}

// 录入验证结果
const submitFillResult = () => {
  const item = currentItem.value
  if (fillForm.result === 'pass') {
    selectedTask.value.passedCount++
    message.success(\`第 \${item.sequence} 件验证合格 ✅\`)
  } else {
    selectedTask.value.passedCount = 0 // 归零
    message.error(\`第 \${item.sequence} 件不合格！连续合格计数已归零。\`)
  }
...
const confirmRelease = () => {
  Modal.confirm({
    title: '确认放行？',
    content: '验证已达标，系统将向 MES 发送解锁指令，恢复机台正常生产。',
    okText: '确认放行',
    onOk: () => {
      selectedTask.value.status = 'passed'
      message.success('已向 MES 发送解锁指令，机台恢复正常生产。')
    }
  })
}
\`\`\`
...
**UI 亮点：**
- 四格数据卡（要求/已验/连续/剩余）直观展示进度
- 超时任务红色高亮提醒
- 进度条动态颜色（合格绿色/不合格红色/进行中蓝色）
- 放行按钮二次确认，防止误操作

### 5.4 变化点中央看板 (\`ChangePointDashboard.vue\`)

**核心功能：**
- ✅ 统计卡片（今日新增/待审批/验证中/已闭环）
- ✅ 风险等级分布图（饼图）
- ✅ 变化点趋势折线图（按日期）
- ✅ 实时预警列表（含红色/橙色严重等级）
- ✅ 快速跳转操作
...
// 统计卡片数据
const statsCards = [
  { key: 'today', icon: '📋', label: '今日新增', count: 5, color: '#1890ff' },
  { key: 'pending', icon: '⏳', label: '待审批', count: 3, color: '#faad14' },
  { key: 'verifying', icon: '🔬', label: '验证中', count: 2, color: '#13c2c2' },
  { key: 'closed', icon: '✅', label: '已闭环', count: 12, color: '#52c41a' },
]
...
const alerts = ref<AlertRecord[]>([
  {
    id: 'a1',
    changePointId: '1',
    recordNo: 'CPR-20240305-001',
    alertType: 'blind_move',
    severity: 'red',
    message: '🔴 盲动报工！机台 MC-02 在锁定期间出现了批量报工数据，请立即核查。',
    triggerTime: '2024-03-05T10:30:00',
    isRead: false,
  },
  // ...
])
...
### 5.5 风险矩阵规则配置页 (\`RiskMatrixConfig.vue\`)

**核心功能：**
- ✅ 4M1E 分类 Tab 切换
- ✅ 规则列表表格（子类/关键词/风险级/审批路由/互锁开关）
- ✅ 新增/编辑规则弹窗
- ✅ 启用/停用规则开关
- ✅ 规则优先级排序
...
**表单字段：**
\`\`\`typescript
interface RiskMatrixRuleForm {
  changeType: ChangePointType       // 4M1E 大类
  subType: string                   // 子类型
  keywords: string[]                // 关键词标签列表
  defaultRiskLevel: RiskLevel       // 默认风险等级
  requireQEApproval: boolean        // 需要 QE 审批
  requireDirectorApproval: boolean  // 需要总监审批
  requireMesLock: boolean           // 需要 MES 互锁
  requiredTrialCount: number        // 试产件数
  alertTimeoutHours: number         // 超时阈值（小时）
  description: string               // 规则说明
  isActive: boolean                 // 是否启用
  sortOrder: number                 // 优先级
}
\`\`\`
...
| 方法 | 路径 | 描述 | 请求体 | 响应体 |
|------|------|------|--------|--------|
| \`POST\` | \`/api/v1/change-point/records\` | 创建变化点申报单（触发风险引擎） | \`ChangePointRecord\` | \`{ id, recordNo }\` |
| \`GET\` | \`/api/v1/change-point/records\` | 分页查询变化点台账列表 | \`?status=&riskLevel=&keyword=\` | \`{ total, list[] }\` |
| \`GET\` | \`/api/v1/change-point/records/:id\` | 获取单据详情（含 MES 日志、验证任务） | — | \`ChangePointRecord\` |
| \`PUT\` | \`/api/v1/change-point/records/:id\` | 更新草稿单据内容 | \`Partial<ChangePointRecord>\` | \`{ success: true }\` |
| \`POST\` | \`/api/v1/change-point/records/:id/status\` | 执行状态变更（提交、审批、驳回、闭环） | \`{ action, comment }\` | \`{ success: true }\` |

### 6.2 试生产验证管理

| 方法 | 路径 | 描述 | 请求体 | 响应体 |
|------|------|------|--------|--------|
| \`GET\` | \`/api/v1/change-point/verification-plans\` | 查询试生产验证任务列表 | \`?status=&keyword=\` | \`{ total, list[] }\` |
| \`GET\` | \`/api/v1/change-point/verification-plans/:id\` | 获取验证任务详情（含各件明细） | — | \`VerificationPlan\` |
| \`POST\` | \`/api/v1/change-point/verification-plans/:id/items\` | 录入单件验证结果 | \`{ result, note }\` | \`{ passedCount, completedCount }\` |
| \`POST\` | \`/api/v1/change-point/verification-plans/:id/release\` | 确认放行（触发 MES 解锁） | — | \`{ success: true }\` |

### 6.3 风险矩阵规则管理

| 方法 | 路径 | 描述 | 请求体 | 响应体 |
|------|------|------|--------|--------|
| \`GET\` | \`/api/v1/change-point/risk-matrix/rules\` | 查询风险矩阵规则列表 | \`?changeType=\` | \`{ total, list[] }\` |
| \`POST\` | \`/api/v1/change-point/risk-matrix/rules\` | 新增规则 | \`RiskMatrixRule\` | \`{ id }\` |
| \`PUT\` | \`/api/v1/change-point/risk-matrix/rules/:id\` | 更新规则 | \`Partial<RiskMatrixRule>\` | \`{ success: true }\` |
| \`DELETE\` | \`/api/v1/change-point/risk-matrix/rules/:id\` | 删除规则 | — | \`{ success: true }\` |
| \`POST\` | \`/api/v1/change-point/risk-matrix/evaluate\` | 实时评估（前端提报时预览风险等级） | \`{ changeType, subType, description }\` | \`{ riskLevel, riskScore }\` |

### 6.4 看板与预警

| 方法 | 路径 | 描述 | 请求体 | 响应体 |
|------|------|------|--------|--------|
| \`GET\` | \`/api/v1/change-point/dashboard/stats\` | 看板统计数据（当日/当月汇总） | — | \`DashboardStats\` |
| \`GET\` | \`/api/v1/change-point/alerts\` | 查询预警通知列表 | \`?severity=\` | \`{ total, list[] }\` |
| \`POST\` | \`/api/qms/webhook/mes-violation\` | 接收 MES 盲动报工回调（Webhook） | \`{ machineNo, workOrderId }\` | \`{ success: true }\` |
...
| 风险点 | 严重程度 | 影响范围 | 应对策略 | 责任人 |
|--------|:--------:|----------|----------|--------|
| **风险定级不准确** | 🔴 高 | 变化点可能被错误归类为低风险，跳过验证流程 | ① 定期审核风险矩阵规则准确率；② 关键产品/工序默认提升一级；③ QE 复核高风险样本 | QE 团队 |
| **审批流程卡滞** | 🟡 中 | 变化点长时间未审批，影响生产进度 | ① 超时自动升级（2小时未审批→升级至上级）；② 钉钉实时推送；③ 移动端快速审批 | 生产主管 |
| **MES 互锁失效** | 🔴 高 | 锁机指令未生效，机台继续报工 | ① 锁机后强制二次确认；② MES 回调验证锁机状态；③ 盲动报工立即红色预警 | IT + MES 团队 |
| **验证数据造假** | 🔴 高 | IPQC 虚假录入验证结果 | ① 要求上传现场照片作为附件；② 随机抽查验证记录；③ 连续合格要求合理性校验 | QE 主管 |
| **知识沉淀不足** | 🟡 中 | 闭环后未触发文件更新，知识库无积累 | ① 闭环时强制选择是否触发文件升版；② 定期审核闭环案例质量；③ 知识库推荐命中率统计 | 技术部 |

### 7.2 技术实现风险

| 风险点 | 严重程度 | 影响范围 | 应对策略 | 责任人 |
|--------|:--------:|----------|----------|--------|
| **前后端数据不一致** | 🟡 中 | 前端显示与后端实际状态不同步 | ① 关键操作后强制刷新数据；② 使用乐观锁机制；③ WebSocket 实时推送状态变更 | 前端 + 后端 |
| **并发操作冲突** | 🟡 中 | 多人同时审批/录入导致数据错误 | ① 使用数据库行锁；② 前端禁用重复提交；③ 后端幂等性设计 | 后端团队 |
| **第三方依赖失败** | 🔴 高 | 钉钉推送失败，预警未送达 | ① 降级方案：站内信 + 邮件；② 推送失败自动重试（3次）；③ 监控推送成功率 | IT 运维 |
| **性能瓶颈** | 🟡 中 | 大数据量下列表加载缓慢 | ① 分页查询优化；② 索引优化；③ 前端虚拟滚动 | DBA + 前端 |
| **数据丢失** | 🔴 高 | 系统崩溃导致未保存的数据丢失 | ① 自动保存草稿（30秒）；② 定期数据备份；③ 操作日志审计追踪 | 后端团队 |

### 7.3 用户体验风险

| 风险点 | 严重程度 | 影响范围 | 应对策略 | 责任人 |
|--------|:--------:|----------|----------|--------|
| **学习成本高** | 🟡 中 | 新用户不熟悉系统操作 | ① 提供操作视频教程；② 关键字段悬浮提示；③ 引导式首次使用流程 | 产品经理 |
| **移动端体验差** | 🟡 中 | PDA 现场操作不流畅 | ① 简化移动端功能；② 大按钮设计；③ 离线缓存支持 | 前端团队 |
| **反馈不及时** | 🟢 低 | 用户不清楚操作是否成功 | ① 所有操作提供成功/失败提示；② 长时间操作显示进度条 | 前端团队 |
...
| 测试项 | 测试用例 | 预期结果 | 验证方法 | 状态 |
|--------|----------|----------|----------|:----:|
| **4M1E 分类选择** | 选择"机(Machine)"→"模具更换" | 自动填充子类型选项 | 手动测试 | ✅ |
| **风险评估计算** | 输入"大修"关键词 | 风险等级显示为"高风险" | 自动化测试 | ✅ |
| **草稿保存** | 填写部分内容后点击"暂存草稿" | 数据保存，状态为 draft | 手动测试 | ✅ |
| **提报提交** | 点击"提交申报" | 状态流转为 submitted | 手动测试 | ✅ |
| **审批流程** | QE 点击"批准通过" | 中高风险自动锁机，低风险直接放行 | 集成测试 | ✅ |

#### 8.1.2 MES 互锁测试

| 测试项 | 测试用例 | 预期结果 | 验证方法 | 状态 |
|--------|----------|----------|----------|:----:|
| **自动锁机** | 中风险变化点审批通过 | 调用 MES lock-machine API | Mock 测试 | ✅ |
| **锁机日志记录** | 锁机成功后查询 MES 日志 | cp_mes_lock_log 新增 lock 记录 | 数据库验证 | ✅ |
| **解锁触发** | 验证达标后点击"确认放行" | 调用 MES unlock-machine API | 集成测试 | ⏳ |
| **盲动报工预警** | 锁定机台出现批量报工 | 触发红色预警，钉钉推送 | 模拟回调 | ⏳ |
| **解锁失败回滚** | MES 解锁 API 返回失败 | 单据状态不更新，记录失败日志 | 异常测试 | ⏳ |

#### 8.1.3 验证任务测试

| 测试项 | 测试用例 | 预期结果 | 验证方法 | 状态 |
|--------|----------|----------|----------|:----:|
| **验证任务生成** | 中高风险审批通过 | 自动创建 verification_plan 记录 | 数据库验证 | ✅ |
| **结果录入** | IPQC 录入第1件结果为"合格" | passedCount = 1 | 手动测试 | ✅ |
| **归零逻辑** | 第2件录入"不合格" | passedCount 归零为 0 | 手动测试 | ✅ |
| **放行条件** | passedCount >= requiredCount | 出现"确认放行"按钮 | 手动测试 | ✅ |
| **超时预警** | 验证超过 4 小时未完成 | 生成橙色预警记录 | 定时任务测试 | ⏳ |

#### 8.1.4 闭环与知识沉淀测试

| 测试项 | 测试用例 | 预期结果 | 验证方法 | 状态 |
|--------|----------|----------|----------|:----:|
| **闭环操作** | 点击"标记闭环"，填写说明 | 状态更新为 closed | 手动测试 | ✅ |
| **文件升版提示** | 勾选"触发关联文件升版" | 提示技术部门 | 手动测试 | ⏳ |
| **知识库沉淀** | 闭环完成 | saved_to_knowledge = TRUE | 数据库验证 | ⏳ |

### 8.2 性能测试

| 测试项 | 测试条件 | 性能指标 | 验证方法 | 状态 |
|--------|----------|----------|----------|:----:|
| **列表加载** | 1000 条变化点记录 | 首次加载 < 1s | 性能测试工具 | ⏳ |
| **搜索响应** | 关键字搜索 | 响应时间 < 500ms | 自动化测试 | ⏳ |
| **并发审批** | 100 个并发审批请求 | 平均响应时间 < 2s | 压力测试 | ⏳ |
| **大数据量导出** | 导出 10000 条记录 | 导出时间 < 30s | 手动测试 | ⏳ |

### 8.3 安全测试

| 测试项 | 测试用例 | 预期结果 | 验证方法 | 状态 |
|--------|----------|----------|----------|:----:|
| **权限控制** | 普通用户访问风险矩阵配置页 | 拒绝访问 | 权限测试 | ⏳ |
| **SQL 注入** | 在搜索框输入 SQL 语句 | 无异常，安全过滤 | 安全扫描 | ⏳ |
| **XSS 攻击** | 在备注字段输入 JavaScript 代码 | 转义显示，不执行 | 安全扫描 | ⏳ |
| **CSRF 防护** | 跨站请求伪造攻击 | 请求被拒绝 | 安全测试 | ⏳ |

### 8.4 用户验收测试（UAT）

**测试计划：**
1. **测试周期：** 2 周
2. **测试人员：** 品质部 QE、生产部班组长、IPQC 检验员
3. **测试场景：**
   - 场景1：新员工上岗（低风险）
   - 场景2：关键设备大修（高风险）
   - 场景3：供应商切换（中风险）
...
| 优化项 | 优先级 | 预估工作量 | 业务价值 | 负责人 |
|--------|:------:|-----------|----------|--------|
| **移动端 PDA 适配** | 🔴 高 | 2 周 | 现场操作效率提升 50% | 前端团队 |
| **钉钉实时推送集成** | 🔴 高 | 1 周 | 审批及时性提升 70% | 后端团队 |
| **批量导入功能** | 🟡 中 | 1 周 | 减少手工录入工作量 | 后端团队 |
| **数据导出优化** | 🟡 中 | 3 天 | 支持 Excel/CSV 多格式 | 前端团队 |
| **操作引导提示** | 🟢 低 | 3 天 | 降低学习成本 | 产品经理 |

#### 9.1.2 中期优化（3-6个月）

| 优化项 | 优先级 | 预估工作量 | 业务价值 | 负责人 |
|--------|:------:|-----------|----------|--------|
| **知识库智能推荐** | 🟡 中 | 3 周 | 历史案例复用率提升 30% | AI 团队 |
| **风险预测模型** | 🟡 中 | 4 周 | 提前识别高风险变化点 | 数据科学团队 |
| **多语言支持** | 🟢 低 | 2 周 | 支持海外工厂使用 | 前端团队 |
| **离线模式** | 🟢 低 | 3 周 | 网络不稳定环境支持 | 前端团队 |
| **审计报告生成** | 🟡 中 | 2 周 | 自动生成 IATF 审计所需报告 | 后端团队 |

#### 9.1.3 长期规划（6-12个月）

| 优化项 | 优先级 | 预估工作量 | 业务价值 | 负责人 |
|--------|:------:|-----------|----------|--------|
| **AI 辅助风险评估** | 🟡 中 | 2 个月 | 准确率提升至 90% | AI 团队 |
| **跨系统集成** | 🟡 中 | 1.5 个月 | ERP/SRM 数据打通 | 集成团队 |
| **高级分析看板** | 🟢 低 | 1 个月 | 管理层决策支持 | BI 团队 |
| **区块链追溯** | 🟢 低 | 3 个月 | 不可篡改的审计追踪 | 技术架构团队 |
...
| 优化项 | 当前状态 | 目标状态 | 实施方案 |
|--------|----------|----------|----------|
| **状态管理** | 组件内部 reactive | Pinia 全局状态 | 迁移至 Pinia，支持跨组件共享 |
| **API 调用** | 分散在各组件 | 统一 API 层 | 创建 \`src/api/change-point.ts\` |
| **Mock 数据** | 硬编码在组件 | Mock Server | 使用 Mock.js 或 JSON Server |
| **性能优化** | 全量渲染 | 虚拟滚动 | 引入 vue-virtual-scroller |
| **错误处理** | 分散的 try-catch | 统一拦截器 | Axios 拦截器 + 全局错误提示 |

**代码示例（API 层封装）：**
\`\`\`typescript
// src/api/change-point.ts
import request from '@/utils/request'
import type { ChangePointRecord, VerificationPlan } from '@/types/change-point'

export const changePointApi = {
  // 获取变化点列表
  getRecords: (params: any) => 
    request.get<{ total: number; list: ChangePointRecord[] }>('/api/v1/change-point/records', { params }),
  
  // 创建变化点
  createRecord: (data: Partial<ChangePointRecord>) => 
    request.post<{ id: string; recordNo: string }>('/api/v1/change-point/records', data),
  
  // 审批操作
  approve: (id: string, data: { action: string; comment: string }) => 
    request.post(\`/api/v1/change-point/records/\${id}/status\`, data),
  
  // 录入验证结果
  submitVerification: (planId: string, itemId: string, data: { result: string; note: string }) => 
    request.post(\`/api/v1/change-point/verification-plans/\${planId}/items\`, { itemId, ...data }),
}
\`\`\`
...
| 优化项 | 当前状态 | 目标状态 | 实施方案 |
|--------|----------|----------|----------|
| **数据库索引** | 基础索引 | 复合索引优化 | 在 \`status\`, \`risk_level\`, \`report_time\` 上建索引 |
| **缓存策略** | 无缓存 | Redis 缓存 | 缓存热点数据（看板统计、风险规则） |
| **消息队列** | 同步处理 | 异步解耦 | 使用 RabbitMQ 处理 MES 回调、钉钉推送 |
| **定时任务** | 单机 Cron | 分布式调度 | 使用 XXL-JOB 统一调度超时扫描 |
| **监控告警** | 无监控 | Prometheus + Grafana | 监控 API 响应时间、错误率、业务指标 |

### 9.3 数据治理优化

| 优化项 | 当前状态 | 目标状态 | 实施方案 |
|--------|----------|----------|----------|
| **数据质量** | 手工录入 | 智能校验 | 关键字段必填、格式校验、逻辑一致性检查 |
| **数据归档** | 无归档策略 | 定期归档 | 超过 1 年的已闭环记录归档至历史库 |
| **数据分析** | 无分析 | BI 看板 | 风险分布、处理时效、验证通过率分析 |
| **数据备份** | 手工备份 | 自动备份 | 每日自动备份，保留 30 天 |

---

## 10. 实施路线图

### 10.1 第一阶段：核心功能完善（1个月）

**目标：** 完成后端 API 对接，实现完整业务闭环

**任务清单：**
- [ ] 后端 API 开发（变化点 CRUD、审批流程、验证任务）
- [ ] 前端 API 层封装，替换 Mock 数据
- [ ] MES 互锁接口联调
- [ ] 钉钉推送集成
- [ ] 数据库表结构创建与索引优化
- [ ] UAT 测试与 Bug 修复
...
### 10.2 第二阶段：移动端与预警机制（1个月）

**目标：** 完成移动端 PDA 适配，实现实时预警

**任务清单：**
- [ ] PDA 移动端页面开发（申报、验证、审批）
- [ ] 钉钉小程序集成
- [ ] 盲动报工预警机制实现
- [ ] 验证超时预警定时任务
- [ ] WebSocket 实时推送
...
### 10.3 第三阶段：高级功能与优化（2个月）

**目标：** 完成知识库沉淀、智能推荐、性能优化
...
| 类别 | 要求 | 验证方法 |
|---|---|---|
| **安全** | 风险矩阵规则配置页仅超级管理员/品质工程师角色可访问；放行操作需要二次确认弹窗 | 权限测试 + 安全扫描 |
| **数据完整性** | MES 互锁成功响应后才允许更新单据状态；若 MES 接口调用失败，系统记录失败日志并告警，不自动修改状态 | 集成测试 + 异常测试 |
| **审计追溯** | 所有状态变更、审批操作、MES 互锁动作必须完整记录于对应日志表，永久保留不可删除 | 数据库审计 + 日志分析 |
| **超时容错** | MES API 调用超时阈值为 10 秒；若超时，系统自动重试最多 3 次，并在最终失败后触发站内告警 | 压力测试 + 监控告警 |
| **预警实时性** | 盲动报工预警必须在 MES 回调后 30 秒内完成推送；超时验证扫描间隔不超过 15 分钟 | 性能测试 + 监控统计 |
| **可用性** | 系统可用性 ≥ 99.5%；关键业务流程无单点故障 | 容灾演练 + 监控统计 |
| **可扩展性** | 支持横向扩展，单机 QPS ≥ 100；支持 10+ 工厂并发使用 | 性能测试 + 架构评估 |
...
| 术语 | 英文 | 解释 |
|------|------|------|
| 4M1E | Man, Machine, Material, Method, Environment, Measure | 制造业五大要素：人、机、料、法、环、测 |
| MES | Manufacturing Execution System | 制造执行系统 |
| QMS | Quality Management System | 质量管理系统 |
| QE | Quality Engineer | 品质工程师 |
| IPQC | In-Process Quality Control | 制程检验员 |
| SOP | Standard Operating Procedure | 标准作业程序 |
| CP | Control Plan | 控制计划 |
| FMEA | Failure Mode and Effects Analysis | 失效模式与影响分析 |
| IATF 16949 | International Automotive Task Force | 国际汽车工作组质量管理体系标准 |

### 12.2 参考文档

- IATF 16949:2016 汽车质量管理体系标准
- 《S_QMS 项目 AGENTS.md 技术规范》
- 《变化点管理提案》 (\`openspec/changes/add-change-point-management/proposal.md\`)
- 《变化点管理任务清单》 (\`openspec/changes/add-change-point-management/tasks.md\`)
...
*本文档将随着项目进展持续更新，最新版本请参考项目文档库。*
`,te=`# 变化点管理（Change Point Management）详细设计文档 V3.0
...
在汽车零配件制造过程中，4M1E（人、机、料、法、环、测）的变化是引发批量质量问题的核心根因。传统纸质审批流程存在四大痛点：

- **信息孤岛**：变化申报与MES系统割裂，审批通过后无法阻止未验证物料进入批量生产
- **流程断档**：中高风险变化缺乏强制验证关卡，凭经验判断放行
- **预警滞后**：机台被绕开时无实时监控和推送机制
- **知识流失**：处理案例分散，无法形成质量知识库

### 1.2 核心需求

本模块需要实现**端到端的变化点数字化管控体系**，包含六大核心机制：

1. **结构化提报**：4M1E分类 + 详细描述 + 影响范围
2. **智能风险定级**：基于规则引擎自动计算风险等级（低/中/高）
3. **分层审批路由**：根据风险等级动态路由至班组长/QE/品质总监
4. **MES互锁防呆**：中高风险自动锁定机台，禁止批量报工
5. **强制试产验证**：连续合格件数验证，不合格归零重来
6. **知识闭环**：触发文件升版，沉淀至经验库

### 1.3 实现范围

| 端 | 功能范围 | 实现状态 |
|---|---|:---:|
| **PC端（核心）** | 台账列表、申报编辑、验证任务中心、中央看板、规则配置 | ✅ 已实现UI |
| **PDA移动端** | 现场申报、验证录入、快速审批 | ⏳ 待实现 |
...
| 角色 | 职责 | 核心操作 | 使用频率 |
|------|------|----------|----------|
| **一线工人/班组长** | 现场变化发现与申报 | 提报变化点、查看状态、快速审批（低风险） | 高频（3-5次/天） |
| **IPQC检验员** | 现场验证执行 | 录入验证结果、拍照上传、确认放行 | 高频（验证期间） |
| **QE工程师** | 技术评估与审批 | 审批中高风险、制定验证方案、查看看板 | 中频（2-3次/天） |
| **品质经理/总监** | 高风险决策与监控 | 会审高风险、查看看板、处理预警 | 低频（1-2次/天） |
| **系统管理员** | 规则配置与维护 | 配置风险矩阵、查看审计日志、导出数据 | 偶发（1次/周） |
...
**流程：**
1. 班组长在PC端提交"机台MC-02大修"申请，选择"机(Machine)"→"设备大修"
2. 系统根据规则自动定级为**高风险**（命中关键词"大修"）
3. 路由至QE + 品质经理**联合审批**
4. 审批通过后，系统**自动调用MES锁机API**，机台MC-02锁定
5. 生成验证任务：要求连续3件首件检验合格
6. IPQC在现场（PDA）逐件录入验证结果
7. 达标后IPQC点击"确认放行"，系统**自动调用MES解锁API**
8. 班组长标记闭环，触发SOP文件升版

**预期结果：** 防止未验证的大修设备直接批量生产，确保质量稳定
...
**流程：**
1. 班组长提交"操作员A从工位1调整至工位2"申请
2. 系统定级为**低风险**
3. 路由至班组长本人审批（自审）
4. 审批通过后直接放行，**不锁机**
5. 仅记录备案，无需验证

**预期结果：** 快速处理低风险变化，不影响生产节奏

#### 场景3：盲动报工预警（异常处理）

**触发条件：** 机台在锁定状态出现批量报工数据

**流程：**
1. MES系统通过Webhook回调通知QMS：\`POST /api/qms/webhook/mes-violation\`
2. QMS在\`cp_mes_lock_log\`新增记录（action=violation）
3. 生成红色预警：\`🚨 盲动报工！机台MC-02在锁定期间出现批量报工\`
4. **钉钉DING消息**直达生产总监和品质总监
5. 看板首页实时展示红色预警徽章
6. 责任人确认处理后，标记预警为已读

**预期结果：** 30秒内管理层知情，防止质量逃逸
...
graph TB
    subgraph "客户端层"
        PC[PC Web浏览器<br/>Vue3 + Ant Design]
        PDA[PDA移动端<br/>响应式H5]
        DING[钉钉小程序<br/>消息推送]
    end

    subgraph "接入层"
        GW[API网关<br/>Nginx/Kong]
        AUTH[认证中心<br/>JWT Token]
    end

    subgraph "应用层"
        CP[变化点服务<br/>Spring Boot]
        MES_PROXY[MES代理服务<br/>锁机/解锁]
        NOTIFY[通知服务<br/>钉钉/邮件]
    end

    subgraph "数据层"
        MYSQL[(MySQL主库<br/>变化点数据)]
        MYSQL_SLAVE[(MySQL从库<br/>只读查询)]
    ...
    subgraph "外部系统"
        MES[MES系统<br/>生产执行]
        DING_API[钉钉API<br/>消息推送]
        FILE_SERVER[文件服务器<br/>MinIO/OSS]
    end
...
graph LR
    subgraph "视图层"
        LIST[ChangePointList.vue<br/>台账列表]
        EDIT[ChangePointEdit.vue<br/>申报编辑]
        VERIFY[VerificationCenter.vue<br/>验证中心]
        DASHBOARD[ChangePointDashboard.vue<br/>看板]
        CONFIG[RiskMatrixConfig.vue<br/>规则配置]
    end

    subgraph "状态管理层"
        STORE[Pinia Store<br/>全局状态]
        API[API Layer<br/>统一接口]
    end

    subgraph "工具层"
        UTILS[Utils<br/>日期/格式化]
        TYPES[Types<br/>TypeScript定义]
    end
...
Service层 (业务逻辑)
    ├── ChangePointService
    ├── RiskEngineService (风险定级引擎)
    ├── MesInterlockService (MES互锁)
    ├── VerificationService
    ├── NotificationService (钉钉推送)
    └── AuditLogService

Repository层 (数据访问)
    ├── ChangePointRepository
    ├── ApprovalLogRepository
    ├── MesLockLogRepository
    └── VerificationRepository

Integration层 (外部集成)
    ├── MesClient (MES API客户端)
    ├── DingTalkClient (钉钉API客户端)
    └── FileStorageClient (文件存储)
\`\`\`
...
\`\`\`mermaid
erDiagram
    ChangePointRecord ||--|| ChangeDetail : contains
    ChangePointRecord ||--o{ ApprovalLog : has
    ChangePointRecord ||--o{ MesLockLog : has
    ChangePointRecord ||--o| VerificationPlan : triggers
    VerificationPlan ||--o{ VerificationItem : contains
    ChangePointRecord ||--o{ AlertRecord : generates
    RiskMatrixRule ||--o{ ChangePointRecord : evaluates

    ChangePointRecord {
        uuid id PK
        string record_no UK "单号"
        string title
    ...
    ChangeDetail {
        uuid id PK
        uuid record_id FK
        string change_type "4M1E"
        string change_sub_type
        text change_description
        string affected_machine
    }

    VerificationPlan {
        uuid id PK
        uuid record_id FK
        int required_count "要求合格件数"
        int passed_count "连续合格计数"
        string status "pending/running/passed"
        datetime deadline
    }
    ...
    MesLockLog {
        uuid id PK
        uuid record_id FK
        string action "lock/unlock/violation"
        string machine_no
...
**基础URL：** \`https://api.qms.com/v1/change-point\`
...
\`\`\`json
{
  ...
  "changeDetail": {
    "changeType": "machine",
    "changeSubType": "模具更换",
    "changeDescription": "压铸模具A长期使用老化，本次更换新模具...",
    "affectedMachine": "MC-02",
    "affectedProduct": "产品型号ABC"
  },
  "riskDescription": "模具更换可能影响尺寸精度..."
}
\`\`\`
...
**业务逻辑：**
1. 校验必填字段
2. **调用风险定级引擎**：基于规则计算 \`riskLevel\` 和 \`riskScore\`
3. 生成单号：\`CPR-YYYYMMDD-NNN\`（全局唯一）
4. 状态设置为 \`submitted\`
5. 根据风险等级**自动路由审批流**：
   - low → 班组长
   - medium → QE
...
if (action == "approved") {
    if (riskLevel in ["medium", "high"]) {
        // 1. 调用MES锁机API
        MesLockResult result = mesClient.lockMachine(machineNo, recordNo);
        
        // 2. 记录MES日志
        mesLockLogRepository.save(action="lock", result);
        
        // 3. 更新状态为locked
        record.setStatus("locked");
        record.setMesLockTime(now());
        
        // 4. 创建验证任务
        VerificationPlan plan = createVerificationPlan(record);
        
        // 5. 推送通知给IPQC
        notificationService.notifyIPQC(plan);
    } else {
        // 低风险直接放行
...
**POST** \`/verification-plans/{planId}/items\`
...
\`\`\`json
{
  ...
  "data": {
    ...
    "completedCount": 3,
    "requiredCount": 3,
    "canRelease": true  // passedCount >= requiredCount
  }
}
\`\`\`
...
// 关键逻辑：连续合格计数
if (result == "pass") {
    plan.setPassedCount(plan.getPassedCount() + 1);
} else {
    plan.setPassedCount(0);  // 归零
    plan.setConsecutiveFailCount(plan.getConsecutiveFailCount() + 1);
    
    // 连续失败3次自动中止
    if (plan.getConsecutiveFailCount() >= 3) {
...
**POST** \`/verification-plans/{planId}/release\`

**前置校验：**
\`\`\`java
if (plan.getPassedCount() < plan.getRequiredCount()) {
    throw new BusinessException("验证未达标，无法放行");
}
...
\`\`\`java
// 1. 调用MES解锁API
MesUnlockResult result = mesClient.unlockMachine(machineNo, recordNo);

// 2. 记录MES日志
mesLockLogRepository.save(action="unlock", result);

// 3. 更新状态
plan.setStatus("passed");
record.setStatus("released");
record.setMesUnlockTime(now());

// 4. 异步通知生产部门
notificationService.notifyProduction(record);
\`\`\`

#### 6.2.5 MES盲动报工回调

**POST** \`/webhook/mes-violation\`

**请求体（MES推送）：**
\`\`\`json
{
  "machineNo": "MC-02",
  "workOrderId": "WO-20260306-123",
  "reportTime": "2026-03-06T14:30:00Z",
  "quantity": 50
}
\`\`\`
...
\`\`\`java
// 1. 查询该机台是否有锁定中的变化点
ChangePointRecord record = repository.findByMachineAndStatus(machineNo, ["locked", "verifying"]);

if (record != null) {
    // 2. 记录违规日志
    mesLockLogRepository.save(action="violation", note="盲动报工，数量:" + quantity);
    
    // 3. 生成红色预警
    AlertRecord alert = new AlertRecord();
    alert.setAlertType("blind_move");
    alert.setSeverity("red");
    alert.setMessage("🔴 盲动报工！机台" + machineNo + "在锁定期间报工" + quantity + "件");
    alertRepository.save(alert);
    
    // 4. 钉钉DING消息直达管理层
    notificationService.sendDingDing(alert, ["生产总监", "品质总监"]);
}
\`\`\`

### 6.3 WebSocket实时推送

**连接URL：** \`wss://api.qms.com/ws\`

**订阅主题：**
\`\`\`json
{
  "action": "subscribe",
  "topics": ["alerts", "verification-updates"]
}
\`\`\`
...
\`\`\`json
{
  ...
  "severity": "red",
  "title": "盲动报工预警",
  "message": "机台MC-02在锁定期间出现批量报工",
  "timestamp": 1709721600000
}
\`\`\`
...
\`\`\`sql
CREATE TABLE \`cp_record\` (
  \`id\` VARCHAR(36) NOT NULL COMMENT '主键UUID',
  \`record_no\` VARCHAR(50) NOT NULL COMMENT '单号 CPR-YYYYMMDD-NNN',
  \`title\` VARCHAR(200) NOT NULL COMMENT '标题',
  \`org_id\` VARCHAR(36) COMMENT '组织ID',
  
  -- 风险评估
  \`risk_level\` VARCHAR(10) NOT NULL COMMENT '风险等级 low/medium/high',
  \`risk_score\` INT COMMENT '风险得分 1-100',
  ...
  -- 提报信息
  \`reporter_id\` VARCHAR(36) NOT NULL COMMENT '提报人ID',
  \`reporter_name\` VARCHAR(50) NOT NULL COMMENT '提报人姓名',
  \`reporter_dept\` VARCHAR(100) COMMENT '提报人部门',
  \`report_time\` DATETIME NOT NULL COMMENT '提报时间',
  ...
  -- MES互锁
  \`mes_lock_time\` DATETIME COMMENT 'MES锁机时间',
  \`mes_unlock_time\` DATETIME COMMENT 'MES解锁时间',
  
  -- 闭环信息
  \`closed_by\` VARCHAR(36) COMMENT '闭环人ID',
  \`close_time\` DATETIME COMMENT '闭环时间',
  \`close_comment\` TEXT COMMENT '闭环说明',
  \`saved_to_knowledge\` BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否沉淀知识库',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='变化点处理单主表';
\`\`\`

#### 验证任务表：\`cp_verification_plan\`

\`\`\`sql
CREATE TABLE \`cp_verification_plan\` (
  \`id\` VARCHAR(36) NOT NULL,
  \`record_id\` VARCHAR(36) NOT NULL COMMENT '关联变化点ID',
  \`plan_title\` VARCHAR(200) NOT NULL COMMENT '验证方案标题',
  
  -- 验证指标
  \`required_count\` INT NOT NULL COMMENT '要求连续合格件数',
  \`completed_count\` INT NOT NULL DEFAULT 0 COMMENT '已验证件数',
  \`passed_count\` INT NOT NULL DEFAULT 0 COMMENT '当前连续合格计数',
  \`consecutive_fail_count\` INT NOT NULL DEFAULT 0 COMMENT '连续失败计数',
  
  -- 责任人
  \`verifier_id\` VARCHAR(36) NOT NULL COMMENT 'IPQC ID',
  \`verifier_name\` VARCHAR(50) NOT NULL COMMENT 'IPQC姓名',
  \`qe_approver_id\` VARCHAR(36) COMMENT 'QE确认人ID',
  ...
  FOREIGN KEY (\`record_id\`) REFERENCES \`cp_record\`(\`id\`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='试生产验证任务表';
\`\`\`
...
| 表名 | 索引名 | 字段 | 类型 | 用途 |
|------|--------|------|------|------|
| \`cp_record\` | \`idx_status_report_time\` | \`(status, report_time)\` | 复合索引 | 列表页状态+时间筛选 |
| \`cp_record\` | \`idx_reporter_id\` | \`reporter_id\` | 单列索引 | 我的申报查询 |
| \`cp_verification_plan\` | \`idx_deadline_status\` | \`(deadline, status)\` | 复合索引 | 超时扫描定时任务 |
| \`cp_mes_lock_log\` | \`idx_record_action\` | \`(record_id, action)\` | 复合索引 | MES日志查询 |
| \`cp_alert_record\` | \`idx_severity_read\` | \`(severity, is_read)\` | 复合索引 | 未读预警查询 |

### 7.3 缓存策略

| 数据类型 | 缓存介质 | 过期时间 | 更新策略 |
|----------|----------|----------|----------|
| **风险矩阵规则** | Redis | 1小时 | 规则修改时主动失效 |
| **看板统计数据** | Redis | 5分钟 | 定时任务刷新 |
| **用户权限信息** | Redis | 30分钟 | 用户登录时加载 |
| **单据详情** | 本地内存 | 5分钟 | LRU缓存，最多1000条 |
| **验证任务进度** | Redis | 实时 | 每次录入时更新 |

**Redis缓存Key设计：**
\`\`\`
cp:risk:rules:{changeType}          # 风险规则
cp:dashboard:stats:{orgId}          # 看板统计
cp:user:permissions:{userId}        # 用户权限
cp:verification:progress:{planId}   # 验证进度
\`\`\`

### 7.4 数据归档策略

**归档规则：**
- 已闭环超过1年的记录 → 归档至历史库 \`cp_record_archive\`
- 每月1日执行归档任务
- 归档后原表保留摘要信息（id, record_no, status, close_time）

**归档表结构：**
\`\`\`sql
CREATE TABLE \`cp_record_archive\` LIKE \`cp_record\`;
ALTER TABLE \`cp_record_archive\` MODIFY \`id\` VARCHAR(36) NOT NULL;
\`\`\`
...
sequenceDiagram
    ...
    User->>Frontend: 填写变化点信息
    Frontend->>API: POST /records (提交申报)
    API->>Engine: evaluateRisk(changeType, subType, description)
    
    Engine->>RuleCache: 获取规则列表
    RuleCache-->>Engine: 返回缓存规则（若miss则查DB）
    
    Engine->>Engine: 精确匹配（changeType+subType）
    Engine->>Engine: 关键词匹配（description扫描）
    Engine->>Engine: 聚合结果（取最高风险级）
    
    Engine-->>API: {riskLevel: "high", riskScore: 85}
    
    API->>DB: 保存记录（status=submitted）
    API->>API: 路由审批流（high → QE+总监）
    API-->>Frontend: {recordNo: "CPR-xxx", riskLevel: "high"}
...
\`\`\`mermaid
sequenceDiagram
    participant Approver as 审批人(QE)
    participant QMS as QMS系统
    participant MES as MES系统
    ...
    Approver->>QMS: POST /records/{id}/approve (通过)
    QMS->>QMS: 检查风险等级=high
    QMS->>MES: POST /mes/machine/lock (锁机)
    MES-->>QMS: {success: true, lockId: "xxx"}
    
    QMS->>DB: 记录MES日志(action=lock)
    QMS->>DB: 更新状态=locked
    QMS->>DB: 创建验证任务
    
    QMS->>MQ: 推送通知消息
    MQ->>DingTalk: 异步发送钉钉通知
    MQ->>IPQC: WebSocket实时推送
    
    DingTalk-->>IPQC: 收到验证任务通知
    
    loop 逐件验证
        IPQC->>QMS: POST /verification-plans/{id}/items (录入结果)
        QMS->>DB: 更新passedCount/completedCount
        
        alt 出现不合格
            QMS->>DB: passedCount归零
            QMS-->>IPQC: 告警"连续计数中断"
        end
    end
    
    IPQC->>QMS: POST /verification-plans/{id}/release (确认放行)
    QMS->>QMS: 校验 passedCount >= requiredCount
    QMS->>MES: POST /mes/machine/unlock (解锁)
    MES-->>QMS: {success: true}
    
    QMS->>DB: 记录MES日志(action=unlock)
    QMS->>DB: 更新状态=released
    QMS-->>IPQC: 放行成功
\`\`\`
...
sequenceDiagram
    participant MES as MES系统
    participant Webhook as QMS Webhook
    participant DB as 数据库
    participant Alert as 预警服务
    participant DingTalk as 钉钉API
    participant Manager as 生产/品质总监

    MES->>Webhook: POST /webhook/mes-violation (报工数据)
    Webhook->>DB: 查询机台锁定状态
    
    alt 机台处于locked/verifying状态
        DB-->>Webhook: 返回锁定记录
        
        Webhook->>DB: 插入MES日志(action=violation)
        Webhook->>DB: 插入预警记录(severity=red)
        
        Webhook->>Alert: 触发红色预警
        Alert->>DingTalk: 发送DING消息（高优先级）
        DingTalk-->>Manager: 手机收到红色预警
        
        Alert->>WebSocket: 实时推送至看板
        WebSocket-->>Manager: 看板显示红色徽章
...
| 技术 | 版本 | 选型理由 |
|------|------|----------|
| **Vue 3** | 3.4+ | 组合式API更灵活，TypeScript支持好，生态成熟 |
| **TypeScript** | 5.2+ | 类型安全，减少运行时错误，提升代码质量 |
| **Ant Design Vue** | 4.2+ | 企业级UI组件库，符合QMS系统严肃风格，组件丰富 |
| **Pinia** | 2.1+ | 新一代状态管理，TypeScript友好，比Vuex更简洁 |
| **Vue Router** | 4.2+ | 官方路由方案，支持路由守卫和懒加载 |
| **Axios** | 1.6+ | HTTP客户端，拦截器机制方便统一处理请求 |
| **dayjs** | 1.11+ | 轻量级日期库（2KB），比moment小96% |
| **ECharts** | 5.4+ | 专业图表库，看板数据可视化必需 |
| **Vite** | 5.0+ | 极速开发体验，HMR秒级热更新 |

**关键设计决策：**
- ✅ **组合式API**：逻辑复用更灵活，适合复杂业务组件
- ✅ **严格TypeScript**：启用\`strict\`模式，杜绝\`any\`类型
- ✅ **按需加载**：路由级代码分割，首屏加载优化
...
| 技术 | 版本 | 选型理由 |
|------|------|----------|
| **Spring Boot** | 3.2+ | Java生态最成熟的微服务框架，企业级首选 |
| **Spring Security** | 6.2+ | 安全框架，JWT认证 + RBAC权限控制 |
| **MyBatis-Plus** | 3.5+ | ORM框架，简化CRUD操作，代码生成器 |
| **MySQL** | 8.0+ | 主流关系数据库，事务支持完善，生态成熟 |
| **Redis** | 7.0+ | 缓存中间件，规则/统计/会话缓存 |
| **RabbitMQ** | 3.12+ | 消息队列，异步解耦（通知、MES回调） |
| **Springdoc** | 2.3+ | OpenAPI 3.0文档生成，替代Swagger |
| **JUnit 5** | 5.10+ | 单元测试框架，参数化测试支持好 |
| **Docker** | 24+ | 容器化部署，环境一致性保障 |
| **Prometheus + Grafana** | - | 监控告警，实时性能指标可视化 |

**架构原则：**
- ✅ **分层架构**：Controller → Service → Repository 清晰分层
- ✅ **依赖倒置**：Service依赖接口而非实现，方便测试和扩展
- ✅ **单一职责**：每个Service只负责一个业务领域
- ✅ **接口隔离**：API设计遵循RESTful规范
...
|------|------|------|------|
| **开发环境** | 4C8G | 1台 | 本地开发调试 |
| **测试环境** | 8C16G | 2台 | 功能测试、集成测试 |
| **预发布环境** | 8C16G | 2台 | UAT验收、性能测试 |
| **生产环境** | 16C32G | 3台 | 高可用部署（主从+读写分离） |
...
| 风险点 | 严重程度 | 概率 | 影响范围 | 应对策略 | 责任人 |
|--------|:--------:|:----:|----------|----------|--------|
| **风险定级不准确** | 🔴 高 | 中 | 中低风险被误判为低风险，跳过验证 | ① 关键产品默认提升一级；② QE定期审核准确率；③ 引入AI辅助判断 | QE主管 |
| **审批流程卡滞** | 🟡 中 | 高 | 变化点长时间未审批，影响生产进度 | ① 超时自动升级（2h→上级）；② 钉钉实时推送；③ 移动端快速审批 | 生产经理 |
| **验证数据造假** | 🔴 高 | 低 | IPQC虚假录入，质量隐患流入市场 | ① 强制上传现场照片；② 随机抽查验证；③ 连续合格合理性校验 | 品质总监 |
| **MES互锁失效** | 🔴 高 | 低 | 锁机指令未生效，机台继续报工 | ① 锁机后二次确认；② MES回调验证；③ 盲动报工立即红色预警 | IT + MES团队 |
| **知识沉淀不足** | 🟡 中 | 高 | 闭环后未更新文件，知识库无积累 | ① 闭环时强制选择是否升版；② 定期审核闭环质量 | 技术部 |

### 10.2 技术风险

| 风险点 | 严重程度 | 概率 | 应对策略 |
|--------|:--------:|:----:|----------|
| **第三方依赖失败** | 🔴 高 | 中 | 钉钉/MES接口不可用 → 降级方案：站内信+邮件，推送失败重试3次 |
| **并发操作冲突** | 🟡 中 | 中 | 多人同时审批 → 数据库行锁 + 乐观锁（version字段） |
| **性能瓶颈** | 🟡 中 | 高 | 大数据量查询慢 → 复合索引 + Redis缓存 + 分页优化 |
| **数据丢失** | 🔴 高 | 低 | 系统崩溃 → 自动保存草稿（30s） + 每日备份 + 操作日志审计 |
| **前后端数据不一致** | 🟡 中 | 中 | 显示状态与实际不符 → 关键操作后强制刷新 + WebSocket实时推送 |
...
**QMS → MES（主动调用）：**
\`\`\`
POST /api/mes/machine/lock
Body: { machineNo, workOrderId, reason, lockId }

POST /api/mes/machine/unlock
Body: { machineNo, lockId, verificationPlanId }
\`\`\`

**MES → QMS（回调通知）：**
\`\`\`
POST /api/qms/webhook/mes-violation
Body: { machineNo, workOrderId, reportTime, quantity }
\`\`\`

**接口超时与重试：**
- 超时时间：10秒
- 重试策略：失败后立即重试，最多3次，间隔1s/2s/5s
- 失败处理：记录失败日志，触发站内告警，不自动修改单据状态
...
1. **审批通知**（工作消息）
   - 触发：变化点提交后路由至审批人
   - 模板：\`您有一个新的变化点待审批：【标题】风险等级：【高/中/低】\`

2. **DING消息**（高优先级）
   - 触发：盲动报工、验证超时
   - 接收人：生产总监、品质总监
   - 模板：\`🚨【红色预警】机台MC-02在锁定期间出现批量报工，请立即核查！\`

3. **预警催办**（橙色）
   - 触发：验证超时4小时
   - 接收人：验证责任人、QE
   - 模板：\`⏰【超时提醒】变化点CPR-xxx验证已超时，请尽快处理\`
...
Body: {
  ...
  "markdown": {
    "title": "变化点预警",
    "text": "### 🚨 盲动报工预警\\n> 机台：MC-02\\n> 时间：2026-03-06 14:30\\n> 详情：[点击查看](https://qms.com/detail/123)"
  },
  "at": {
    "atUserIds": ["user-001", "user-002"],
    "isAtAll": false
  }
}
\`\`\`
...
| 系统 | 依赖类型 | 用途 | 可用性要求 |
|------|----------|------|-----------|
| **统一认证平台** | 强依赖 | 用户登录、权限校验 | ≥ 99.9% |
| **文件存储服务** | 弱依赖 | 附件上传、照片存储 | 降级时禁用上传 |
| **日志平台** | 弱依赖 | 操作日志收集 | 异步写入，不影响业务 |
| **监控平台** | 弱依赖 | 性能指标采集 | 不影响业务流程 |
...
**当前痛点：** 1000条数据列表加载2-3秒

**优化方案：**
1. **前端虚拟滚动**：引入\`vue-virtual-scroller\`，仅渲染可见行
2. **后端分页优化**：游标分页（cursor-based）替代offset分页
3. **索引优化**：添加复合索引\`(status, report_time)\`
4. **缓存策略**：Redis缓存看板统计（5分钟TTL）

**预期效果：**
- 列表加载时间：2-3s → 0.5s（提升80%）
- API响应时间：1s → 200ms（P95）
...
\`\`\`java
// 风险引擎增强
RiskResult result = riskEngine.evaluate(changePoint);
if (result.getConfidence() < 0.7) {
    // 低置信度时调用AI辅助
    AIRiskPrediction aiResult = aiService.predict(changePoint);
    result.setAiSuggestion(aiResult);
}
\`\`\`
...
**实施方案：**
- 向量化变化描述（BERT/Sentence-BERT）
- 余弦相似度计算（Top 5相似案例）
- 推荐展示：\`相似案例：CPR-202511-xxx（大修换模）→ 验证方案：连续3件首件检验\`

**预期效果：**
- 历史案例复用率：5% → 30%
- 验证方案制定时间：30min → 10min

### 11.3 长期规划（6-12个月）

#### 优化5：跨系统集成

**目标：** 打通ERP/SRM，实现供应链质量追溯

**集成架构：**
\`\`\`
ERP（物料主数据）→ QMS（供应商切换）→ SRM（供应商评估）
                     ↓
                  MES（生产执行）
\`\`\`

**数据同步：**
- ERP物料变更 → 自动触发QMS变化点申报
- QMS闭环评价 → 同步至SRM供应商绩效

#### 优化6：区块链审计追溯

**场景：** IATF 16949审核要求不可篡改的追溯链
...
| 债务项 | 当前状态 | 目标状态 | 预估工作量 |
|--------|----------|----------|-----------|
| **Mock数据迁移** | 组件内硬编码 | Mock Server（JSON Server） | 3人日 |
| **状态管理统一** | 分散在各组件 | Pinia全局Store | 5人日 |
| **API层封装** | 分散调用 | 统一API层 + 拦截器 | 5人日 |
| **单元测试补充** | 覆盖率<20% | 覆盖率≥60% | 10人日 |
| **E2E测试编写** | 无 | Playwright自动化测试 | 8人日 |
...
\`\`\`yaml
# Prometheus指标示例
cp_records_total{status="submitted"}  # 待审批数量
cp_verification_avg_duration_seconds  # 验证平均耗时
cp_mes_lock_success_rate             # MES锁机成功率
cp_alert_unread_count{severity="red"} # 未读红色预警数
\`\`\`
...
- alert: VerificationTimeout
  expr: cp_verification_avg_duration_seconds > 14400  # 4小时
  for: 5m
  annotations:
    summary: "验证任务超时"
    description: "{{ $labels.plan_id }} 已超时 {{ $value }}秒"
\`\`\`
...
const RISK_CONFIG = {
  low:    { label: '低风险', color: '#52c41a', approval: '班组长' },
  medium: { label: '中风险', color: '#faad14', approval: 'QE', mesLock: true },
  high:   { label: '高风险', color: '#ff4d4f', approval: 'QE+总监', mesLock: true }
}
\`\`\`

**状态枚举：**
\`\`\`typescript
type ChangePointStatus =
  | 'draft'      // 草稿
  | 'submitted'  // 已提报
  | 'approving'  // 审批中
  | 'approved'   // 已批准
...
`,oe=`# 不良现象与字典库 (Defect Phenomenon / Cause) - 详细设计文档\r
\r
## 1. 模块概述\r
**模块名称**：质量缺陷与失效原因特征库\r
...
`,ce=`# 抽样方案管理 (Sampling Plan) - 详细设计文档\r
\r
## 1. 模块概述\r
...
`,le=`# 检验方法管理 (Inspection Method) - 详细设计文档\r
\r
## 1. 模块概述\r
...
`,Se="# 量检具台账与类型管理 (Gauge Ledger) - 详细设计文档\r\n\r\n## 1. 模块概述\r\n**模块名称**：量检具管理台账与层级树\r\n**所属子系统**：质量主数据模块 (Quality Master Data) / 检验支持模块\r\n**目标**：构建全厂测量系统资产（卡尺、硬度仪、三坐标等设备）的全生命周期管理跟踪卡。支持设备的层级分类、高精度测量属性配置并核心支撑MSA（测量系统分析）的依赖前提和定期设备送校提醒。\r\n\r\n## 2. 界面布局与交互设计\r\n### 2.1 “左树右表” 复杂布局 (Split Layout)\r\n页面布局采用了高频的高级左树右表结构：\r\n- **左侧容器 (Left Panel - Tree)**：使用 `a-tree` 组建渲染量检具分类树。\r\n  - 支持节点过滤搜索（如输入“卡尺”过滤树状节点）。\r\n  - 在每个节点上支持悬浮动作组（新增子类、编辑本类、删除大类）。\r\n- **右侧工作台 (Right Panel - Ledger List)**：设备明细列表。\r\n  - 只有在选中左侧设备类目后，右侧才会呈现挂属于此品类或此族系下的设备实体。\r\n  - 带有醒目的表头展示目前选中的大类其精度或“是否需MSA”。\r\n\r\n### 2.2 视觉驱动的阈值警告\r\n在量检具中，“校准到期监控”是最为关键的质量管理动作。\r\n- 在表格和详情中渲染 `nextCalDate` 时加入强校验逻辑：\r\n  - 如果日期已逾期 ( `<0 days` )：使用红色警报 `red`\r\n  - 如果在即将逾期范围内 ( `<14 days` )：使用警戒色 `orange`\r\n  - 正常在案：绿色 `green`\r\n\r\n## 3. 核心功能设计\r\n### 3.1 量检具类型属性定义 (Gauge Type Tree)\r\n类型不仅仅是个文本标签，他附带两大质量属性：\r\n- `precision` (分辨率/精度)：如0.01mm，这个精度将直接预设到新建的具体卡尺上。\r\n- `isMsaRequired` (是否需要MSA校验)：布尔值控制，如果是，未来在进行 GR&R 试验分析时，该类的设备才会被强制要求挑选出做验证。\r\n\r\n### 3.2 周期性校验 (Calibration Cycle)\r\n这部分由核心的生命周期字段控制：\r\n- `status`: 在用、校准中、封存、报废。\r\n- 如果设备状态被标记为`CALIBRATING`(校准中)或`SCRAPPED`(报废)，在检验员准备做 IPQC/OQC 选择所用设备输入时，下拉框会对其进行自动防呆拦截，不可选用。\r\n\r\n## 4. 数据模型 (Data Model)\r\n**类型节点实体 (`GaugeType`)**:\r\n- `id`, `parentId`, `typeName`: 维护无限极树的基础。\r\n- `precision`: 缺省精度\r\n- `isMsaRequired`: 强制MSA分析标记\r\n\r\n**量检具台账明细 (`GaugeLedger`)**:\r\n- `gaugeNo`: 设备唯一资产编号\r\n- `gaugeName`, `model`, `manufacturer`: 基本名牌参数\r\n- `accuracy`, `resolution`, `measureRange`: 测定边界能力\r\n- `calibrationCycle`: 周期(比如 12个月)\r\n- `lastCalDate`: 上次强检时间\r\n- `nextCalDate`: 证书失效/下次强检时间(触发报警核心字段)\r\n- `status`: 生命周期（`IN_USE`, `CALIBRATING`, `SEALED`, `SCRAPPED`）\r\n- `custodian`, `location`: 物理存放与保管责任追踪。\r\n\r\n## 5. 约束与规则\r\n- **强校准阻断**：当 `nextCalDate` 已过期，其对应的台账状态建议必须进入锁定态，不得在生产检验流程的下拉列表中再被选中作为“测定依据”，否则极易导致产品错漏判的质量事故。\r\n- **关联删除限制**：如果要删除左侧树节点类型时，系统必须防呆确保其下面未挂靠任何存量（哪怕是报废的）台账。\r\n";function $(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var O=$();function Tn(r){O=r}var y={exec:()=>null};function A(r,n=""){let e=typeof r=="string"?r:r.source,a={replace:(t,s)=>{let o=typeof s=="string"?s:s.source;return o=o.replace(_.caret,"$1"),e=e.replace(t,o),a},getRegex:()=>new RegExp(e,n)};return a}var Ae=(()=>{try{return!!new RegExp("(?<=1)(?<!1)")}catch{return!1}})(),_={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] +\S/,listReplaceTask:/^\[[ xX]\] +/,listTaskCheckbox:/\[[ xX]\]/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:r=>new RegExp(`^( {0,3}${r})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:r=>new RegExp(`^ {0,${Math.min(3,r-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:r=>new RegExp(`^ {0,${Math.min(3,r-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:r=>new RegExp(`^ {0,${Math.min(3,r-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:r=>new RegExp(`^ {0,${Math.min(3,r-1)}}#`),htmlBeginRegex:r=>new RegExp(`^ {0,${Math.min(3,r-1)}}<(?:[a-z].*>|!--)`,"i"),blockquoteBeginRegex:r=>new RegExp(`^ {0,${Math.min(3,r-1)}}>`)},Ee=/^(?:[ \t]*(?:\n|$))+/,Te=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,Ce=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,x=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,ge=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,Z=/ {0,3}(?:[*+-]|\d{1,9}[.)])/,Cn=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,gn=A(Cn).replace(/bull/g,Z).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),_e=A(Cn).replace(/bull/g,Z).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),J=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,Re=/^[^\n]+/,nn=/(?!\s*\])(?:\\[\s\S]|[^\[\]\\])+/,Me=A(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",nn).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),Ie=A(/^(bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,Z).getRegex(),Y="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",en=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,Pe=A("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",en).replace("tag",Y).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),_n=A(J).replace("hr",x).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Y).getRegex(),he=A(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",_n).getRegex(),rn={blockquote:he,code:Te,def:Me,fences:Ce,heading:ge,hr:x,html:Pe,lheading:gn,list:Ie,newline:Ee,paragraph:_n,table:y,text:Re},pn=A("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",x).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Y).getRegex(),De={...rn,lheading:_e,table:pn,paragraph:A(J).replace("hr",x).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",pn).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)])[ \\t]").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Y).getRegex()},Ne={...rn,html:A(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",en).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:y,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:A(J).replace("hr",x).replace("heading",` *#{1,6} *[^
]`).replace("lheading",gn).replace("|table","").replace("blockquote"," {0,3}>").replace("|fences","").replace("|list","").replace("|html","").replace("|tag","").getRegex()},ye=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,Le=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,Rn=/^( {2,}|\\)\n(?!\s*$)/,Oe=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,G=/[\p{P}\p{S}]/u,tn=/[\s\p{P}\p{S}]/u,Mn=/[^\s\p{P}\p{S}]/u,fe=A(/^((?![*_])punctSpace)/,"u").replace(/punctSpace/g,tn).getRegex(),In=/(?!~)[\p{P}\p{S}]/u,ve=/(?!~)[\s\p{P}\p{S}]/u,ke=/(?:[^\s\p{P}\p{S}]|~)/u,Pn=/(?![*_])[\p{P}\p{S}]/u,be=/(?![*_])[\s\p{P}\p{S}]/u,Ve=/(?:[^\s\p{P}\p{S}]|[*_])/u,Qe=A(/link|precode-code|html/,"g").replace("link",/\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-",Ae?"(?<!`)()":"(^^|[^`])").replace("code",/(?<b>`+)[^`]+\k<b>(?!`)/).replace("html",/<(?! )[^<>]*?>/).getRegex(),hn=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,Ue=A(hn,"u").replace(/punct/g,G).getRegex(),xe=A(hn,"u").replace(/punct/g,In).getRegex(),Dn="^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",He=A(Dn,"gu").replace(/notPunctSpace/g,Mn).replace(/punctSpace/g,tn).replace(/punct/g,G).getRegex(),qe=A(Dn,"gu").replace(/notPunctSpace/g,ke).replace(/punctSpace/g,ve).replace(/punct/g,In).getRegex(),we=A("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)","gu").replace(/notPunctSpace/g,Mn).replace(/punctSpace/g,tn).replace(/punct/g,G).getRegex(),Be=A(/^~~?(?:((?!~)punct)|[^\s~])/,"u").replace(/punct/g,Pn).getRegex(),Fe="^[^~]+(?=[^~])|(?!~)punct(~~?)(?=[\\s]|$)|notPunctSpace(~~?)(?!~)(?=punctSpace|$)|(?!~)punctSpace(~~?)(?=notPunctSpace)|[\\s](~~?)(?!~)(?=punct)|(?!~)punct(~~?)(?!~)(?=punct)|notPunctSpace(~~?)(?=notPunctSpace)",Ye=A(Fe,"gu").replace(/notPunctSpace/g,Ve).replace(/punctSpace/g,be).replace(/punct/g,Pn).getRegex(),Ge=A(/\\(punct)/,"gu").replace(/punct/g,G).getRegex(),je=A(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme",/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email",/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),We=A(en).replace("(?:-->|$)","-->").getRegex(),Ke=A("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment",We).replace("attribute",/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),w=/(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/,ze=A(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label",w).replace("href",/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title",/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),Nn=A(/^!?\[(label)\]\[(ref)\]/).replace("label",w).replace("ref",nn).getRegex(),yn=A(/^!?\[(ref)\](?:\[\])?/).replace("ref",nn).getRegex(),Xe=A("reflink|nolink(?!\\()","g").replace("reflink",Nn).replace("nolink",yn).getRegex(),un=/[hH][tT][tT][pP][sS]?|[fF][tT][pP]/,an={_backpedal:y,anyPunctuation:Ge,autolink:je,blockSkip:Qe,br:Rn,code:Le,del:y,delLDelim:y,delRDelim:y,emStrongLDelim:Ue,emStrongRDelimAst:He,emStrongRDelimUnd:we,escape:ye,link:ze,nolink:yn,punctuation:fe,reflink:Nn,reflinkSearch:Xe,tag:Ke,text:Oe,url:y},$e={...an,link:A(/^!?\[(label)\]\((.*?)\)/).replace("label",w).getRegex(),reflink:A(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label",w).getRegex()},K={...an,emStrongRDelimAst:qe,emStrongLDelim:xe,delLDelim:Be,delRDelim:Ye,url:A(/^((?:protocol):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/).replace("protocol",un).replace("email",/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\[\s\S]|[^\\])*?(?:\\[\s\S]|[^\s~\\]))\1(?=[^~]|$)/,text:A(/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|protocol:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/).replace("protocol",un).getRegex()},Ze={...K,br:A(Rn).replace("{2,}","*").getRegex(),text:A(K.text).replace("\\b_","\\b_| {2,}\\n").replace(/\{2,\}/g,"*").getRegex()},H={normal:rn,gfm:De,pedantic:Ne},V={normal:an,gfm:K,breaks:Ze,pedantic:$e},Je={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},mn=r=>Je[r];function P(r,n){if(n){if(_.escapeTest.test(r))return r.replace(_.escapeReplace,mn)}else if(_.escapeTestNoEncode.test(r))return r.replace(_.escapeReplaceNoEncode,mn);return r}function Sn(r){try{r=encodeURI(r).replace(_.percentDecode,"%")}catch{return null}return r}function An(r,n){var s;let e=r.replace(_.findPipe,(o,i,l)=>{let c=!1,p=i;for(;--p>=0&&l[p]==="\\";)c=!c;return c?"|":" |"}),a=e.split(_.splitPipe),t=0;if(a[0].trim()||a.shift(),a.length>0&&!((s=a.at(-1))!=null&&s.trim())&&a.pop(),n)if(a.length>n)a.splice(n);else for(;a.length<n;)a.push("");for(;t<a.length;t++)a[t]=a[t].trim().replace(_.slashPipe,"|");return a}function Q(r,n,e){let a=r.length;if(a===0)return"";let t=0;for(;t<a&&r.charAt(a-t-1)===n;)t++;return r.slice(0,a-t)}function nr(r,n){if(r.indexOf(n[1])===-1)return-1;let e=0;for(let a=0;a<r.length;a++)if(r[a]==="\\")a++;else if(r[a]===n[0])e++;else if(r[a]===n[1]&&(e--,e<0))return a;return e>0?-2:-1}function er(r,n=0){let e=n,a="";for(let t of r)if(t==="	"){let s=4-e%4;a+=" ".repeat(s),e+=s}else a+=t,e++;return a}function En(r,n,e,a,t){let s=n.href,o=n.title||null,i=r[1].replace(t.other.outputLinkReplace,"$1");a.state.inLink=!0;let l={type:r[0].charAt(0)==="!"?"image":"link",raw:e,href:s,title:o,text:i,tokens:a.inlineTokens(i)};return a.state.inLink=!1,l}function rr(r,n,e){let a=r.match(e.other.indentCodeCompensation);if(a===null)return n;let t=a[1];return n.split(`
`).map(s=>{let o=s.match(e.other.beginningSpace);if(o===null)return s;let[i]=o;return i.length>=t.length?s.slice(t.length):s}).join(`
`)}var B=class{constructor(r){C(this,"options");C(this,"rules");C(this,"lexer");this.options=r||O}space(r){let n=this.rules.block.newline.exec(r);if(n&&n[0].length>0)return{type:"space",raw:n[0]}}code(r){let n=this.rules.block.code.exec(r);if(n){let e=n[0].replace(this.rules.other.codeRemoveIndent,"");return{type:"code",raw:n[0],codeBlockStyle:"indented",text:this.options.pedantic?e:Q(e,`
`)}}}fences(r){let n=this.rules.block.fences.exec(r);if(n){let e=n[0],a=rr(e,n[3]||"",this.rules);return{type:"code",raw:e,lang:n[2]?n[2].trim().replace(this.rules.inline.anyPunctuation,"$1"):n[2],text:a}}}heading(r){let n=this.rules.block.heading.exec(r);if(n){let e=n[2].trim();if(this.rules.other.endingHash.test(e)){let a=Q(e,"#");(this.options.pedantic||!a||this.rules.other.endingSpaceChar.test(a))&&(e=a.trim())}return{type:"heading",raw:n[0],depth:n[1].length,text:e,tokens:this.lexer.inline(e)}}}hr(r){let n=this.rules.block.hr.exec(r);if(n)return{type:"hr",raw:Q(n[0],`
`)}}blockquote(r){let n=this.rules.block.blockquote.exec(r);if(n){let e=Q(n[0],`
`).split(`
`),a="",t="",s=[];for(;e.length>0;){let o=!1,i=[],l;for(l=0;l<e.length;l++)if(this.rules.other.blockquoteStart.test(e[l]))i.push(e[l]),o=!0;else if(!o)i.push(e[l]);else break;e=e.slice(l);let c=i.join(`
`),p=c.replace(this.rules.other.blockquoteSetextReplace,`
    $1`).replace(this.rules.other.blockquoteSetextReplace2,"");a=a?`${a}
${c}`:c,t=t?`${t}
${p}`:p;let m=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(p,s,!0),this.lexer.state.top=m,e.length===0)break;let u=s.at(-1);if((u==null?void 0:u.type)==="code")break;if((u==null?void 0:u.type)==="blockquote"){let d=u,T=d.raw+`
`+e.join(`
`),S=this.blockquote(T);s[s.length-1]=S,a=a.substring(0,a.length-d.raw.length)+S.raw,t=t.substring(0,t.length-d.text.length)+S.text;break}else if((u==null?void 0:u.type)==="list"){let d=u,T=d.raw+`
`+e.join(`
`),S=this.list(T);s[s.length-1]=S,a=a.substring(0,a.length-u.raw.length)+S.raw,t=t.substring(0,t.length-d.raw.length)+S.raw,e=T.substring(s.at(-1).raw.length).split(`
`);continue}}return{type:"blockquote",raw:a,tokens:s,text:t}}}list(r){var e,a;let n=this.rules.block.list.exec(r);if(n){let t=n[1].trim(),s=t.length>1,o={type:"list",raw:"",ordered:s,start:s?+t.slice(0,-1):"",loose:!1,items:[]};t=s?`\\d{1,9}\\${t.slice(-1)}`:`\\${t}`,this.options.pedantic&&(t=s?t:"[*+-]");let i=this.rules.other.listItemRegex(t),l=!1;for(;r;){let p=!1,m="",u="";if(!(n=i.exec(r))||this.rules.block.hr.test(r))break;m=n[0],r=r.substring(m.length);let d=er(n[2].split(`
`,1)[0],n[1].length),T=r.split(`
`,1)[0],S=!d.trim(),g=0;if(this.options.pedantic?(g=2,u=d.trimStart()):S?g=n[1].length+1:(g=d.search(this.rules.other.nonSpaceChar),g=g>4?1:g,u=d.slice(g),g+=n[1].length),S&&this.rules.other.blankLine.test(T)&&(m+=T+`
`,r=r.substring(T.length+1),p=!0),!p){let I=this.rules.other.nextBulletRegex(g),f=this.rules.other.hrRegex(g),D=this.rules.other.fencesBeginRegex(g),h=this.rules.other.headingBeginRegex(g),j=this.rules.other.htmlBeginRegex(g),Ln=this.rules.other.blockquoteBeginRegex(g);for(;r;){let W=r.split(`
`,1)[0],k;if(T=W,this.options.pedantic?(T=T.replace(this.rules.other.listReplaceNesting,"  "),k=T):k=T.replace(this.rules.other.tabCharGlobal,"    "),D.test(T)||h.test(T)||j.test(T)||Ln.test(T)||I.test(T)||f.test(T))break;if(k.search(this.rules.other.nonSpaceChar)>=g||!T.trim())u+=`
`+k.slice(g);else{if(S||d.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||D.test(d)||h.test(d)||f.test(d))break;u+=`
`+T}S=!T.trim(),m+=W+`
`,r=r.substring(W.length+1),d=k.slice(g)}}o.loose||(l?o.loose=!0:this.rules.other.doubleBlankLine.test(m)&&(l=!0)),o.items.push({type:"list_item",raw:m,task:!!this.options.gfm&&this.rules.other.listIsTask.test(u),loose:!1,text:u,tokens:[]}),o.raw+=m}let c=o.items.at(-1);if(c)c.raw=c.raw.trimEnd(),c.text=c.text.trimEnd();else return;o.raw=o.raw.trimEnd();for(let p of o.items){if(this.lexer.state.top=!1,p.tokens=this.lexer.blockTokens(p.text,[]),p.task){if(p.text=p.text.replace(this.rules.other.listReplaceTask,""),((e=p.tokens[0])==null?void 0:e.type)==="text"||((a=p.tokens[0])==null?void 0:a.type)==="paragraph"){p.tokens[0].raw=p.tokens[0].raw.replace(this.rules.other.listReplaceTask,""),p.tokens[0].text=p.tokens[0].text.replace(this.rules.other.listReplaceTask,"");for(let u=this.lexer.inlineQueue.length-1;u>=0;u--)if(this.rules.other.listIsTask.test(this.lexer.inlineQueue[u].src)){this.lexer.inlineQueue[u].src=this.lexer.inlineQueue[u].src.replace(this.rules.other.listReplaceTask,"");break}}let m=this.rules.other.listTaskCheckbox.exec(p.raw);if(m){let u={type:"checkbox",raw:m[0]+" ",checked:m[0]!=="[ ]"};p.checked=u.checked,o.loose?p.tokens[0]&&["paragraph","text"].includes(p.tokens[0].type)&&"tokens"in p.tokens[0]&&p.tokens[0].tokens?(p.tokens[0].raw=u.raw+p.tokens[0].raw,p.tokens[0].text=u.raw+p.tokens[0].text,p.tokens[0].tokens.unshift(u)):p.tokens.unshift({type:"paragraph",raw:u.raw,text:u.raw,tokens:[u]}):p.tokens.unshift(u)}}if(!o.loose){let m=p.tokens.filter(d=>d.type==="space"),u=m.length>0&&m.some(d=>this.rules.other.anyLine.test(d.raw));o.loose=u}}if(o.loose)for(let p of o.items){p.loose=!0;for(let m of p.tokens)m.type==="text"&&(m.type="paragraph")}return o}}html(r){let n=this.rules.block.html.exec(r);if(n)return{type:"html",block:!0,raw:n[0],pre:n[1]==="pre"||n[1]==="script"||n[1]==="style",text:n[0]}}def(r){let n=this.rules.block.def.exec(r);if(n){let e=n[1].toLowerCase().replace(this.rules.other.multipleSpaceGlobal," "),a=n[2]?n[2].replace(this.rules.other.hrefBrackets,"$1").replace(this.rules.inline.anyPunctuation,"$1"):"",t=n[3]?n[3].substring(1,n[3].length-1).replace(this.rules.inline.anyPunctuation,"$1"):n[3];return{type:"def",tag:e,raw:n[0],href:a,title:t}}}table(r){var o;let n=this.rules.block.table.exec(r);if(!n||!this.rules.other.tableDelimiter.test(n[2]))return;let e=An(n[1]),a=n[2].replace(this.rules.other.tableAlignChars,"").split("|"),t=(o=n[3])!=null&&o.trim()?n[3].replace(this.rules.other.tableRowBlankLine,"").split(`
`):[],s={type:"table",raw:n[0],header:[],align:[],rows:[]};if(e.length===a.length){for(let i of a)this.rules.other.tableAlignRight.test(i)?s.align.push("right"):this.rules.other.tableAlignCenter.test(i)?s.align.push("center"):this.rules.other.tableAlignLeft.test(i)?s.align.push("left"):s.align.push(null);for(let i=0;i<e.length;i++)s.header.push({text:e[i],tokens:this.lexer.inline(e[i]),header:!0,align:s.align[i]});for(let i of t)s.rows.push(An(i,s.header.length).map((l,c)=>({text:l,tokens:this.lexer.inline(l),header:!1,align:s.align[c]})));return s}}lheading(r){let n=this.rules.block.lheading.exec(r);if(n)return{type:"heading",raw:n[0],depth:n[2].charAt(0)==="="?1:2,text:n[1],tokens:this.lexer.inline(n[1])}}paragraph(r){let n=this.rules.block.paragraph.exec(r);if(n){let e=n[1].charAt(n[1].length-1)===`
`?n[1].slice(0,-1):n[1];return{type:"paragraph",raw:n[0],text:e,tokens:this.lexer.inline(e)}}}text(r){let n=this.rules.block.text.exec(r);if(n)return{type:"text",raw:n[0],text:n[0],tokens:this.lexer.inline(n[0])}}escape(r){let n=this.rules.inline.escape.exec(r);if(n)return{type:"escape",raw:n[0],text:n[1]}}tag(r){let n=this.rules.inline.tag.exec(r);if(n)return!this.lexer.state.inLink&&this.rules.other.startATag.test(n[0])?this.lexer.state.inLink=!0:this.lexer.state.inLink&&this.rules.other.endATag.test(n[0])&&(this.lexer.state.inLink=!1),!this.lexer.state.inRawBlock&&this.rules.other.startPreScriptTag.test(n[0])?this.lexer.state.inRawBlock=!0:this.lexer.state.inRawBlock&&this.rules.other.endPreScriptTag.test(n[0])&&(this.lexer.state.inRawBlock=!1),{type:"html",raw:n[0],inLink:this.lexer.state.inLink,inRawBlock:this.lexer.state.inRawBlock,block:!1,text:n[0]}}link(r){let n=this.rules.inline.link.exec(r);if(n){let e=n[2].trim();if(!this.options.pedantic&&this.rules.other.startAngleBracket.test(e)){if(!this.rules.other.endAngleBracket.test(e))return;let s=Q(e.slice(0,-1),"\\");if((e.length-s.length)%2===0)return}else{let s=nr(n[2],"()");if(s===-2)return;if(s>-1){let o=(n[0].indexOf("!")===0?5:4)+n[1].length+s;n[2]=n[2].substring(0,s),n[0]=n[0].substring(0,o).trim(),n[3]=""}}let a=n[2],t="";if(this.options.pedantic){let s=this.rules.other.pedanticHrefTitle.exec(a);s&&(a=s[1],t=s[3])}else t=n[3]?n[3].slice(1,-1):"";return a=a.trim(),this.rules.other.startAngleBracket.test(a)&&(this.options.pedantic&&!this.rules.other.endAngleBracket.test(e)?a=a.slice(1):a=a.slice(1,-1)),En(n,{href:a&&a.replace(this.rules.inline.anyPunctuation,"$1"),title:t&&t.replace(this.rules.inline.anyPunctuation,"$1")},n[0],this.lexer,this.rules)}}reflink(r,n){let e;if((e=this.rules.inline.reflink.exec(r))||(e=this.rules.inline.nolink.exec(r))){let a=(e[2]||e[1]).replace(this.rules.other.multipleSpaceGlobal," "),t=n[a.toLowerCase()];if(!t){let s=e[0].charAt(0);return{type:"text",raw:s,text:s}}return En(e,t,e[0],this.lexer,this.rules)}}emStrong(r,n,e=""){let a=this.rules.inline.emStrongLDelim.exec(r);if(!(!a||a[3]&&e.match(this.rules.other.unicodeAlphaNumeric))&&(!(a[1]||a[2])||!e||this.rules.inline.punctuation.exec(e))){let t=[...a[0]].length-1,s,o,i=t,l=0,c=a[0][0]==="*"?this.rules.inline.emStrongRDelimAst:this.rules.inline.emStrongRDelimUnd;for(c.lastIndex=0,n=n.slice(-1*r.length+t);(a=c.exec(n))!=null;){if(s=a[1]||a[2]||a[3]||a[4]||a[5]||a[6],!s)continue;if(o=[...s].length,a[3]||a[4]){i+=o;continue}else if((a[5]||a[6])&&t%3&&!((t+o)%3)){l+=o;continue}if(i-=o,i>0)continue;o=Math.min(o,o+i+l);let p=[...a[0]][0].length,m=r.slice(0,t+a.index+p+o);if(Math.min(t,o)%2){let d=m.slice(1,-1);return{type:"em",raw:m,text:d,tokens:this.lexer.inlineTokens(d)}}let u=m.slice(2,-2);return{type:"strong",raw:m,text:u,tokens:this.lexer.inlineTokens(u)}}}}codespan(r){let n=this.rules.inline.code.exec(r);if(n){let e=n[2].replace(this.rules.other.newLineCharGlobal," "),a=this.rules.other.nonSpaceChar.test(e),t=this.rules.other.startingSpaceChar.test(e)&&this.rules.other.endingSpaceChar.test(e);return a&&t&&(e=e.substring(1,e.length-1)),{type:"codespan",raw:n[0],text:e}}}br(r){let n=this.rules.inline.br.exec(r);if(n)return{type:"br",raw:n[0]}}del(r,n,e=""){let a=this.rules.inline.delLDelim.exec(r);if(a&&(!a[1]||!e||this.rules.inline.punctuation.exec(e))){let t=[...a[0]].length-1,s,o,i=t,l=this.rules.inline.delRDelim;for(l.lastIndex=0,n=n.slice(-1*r.length+t);(a=l.exec(n))!=null;){if(s=a[1]||a[2]||a[3]||a[4]||a[5]||a[6],!s||(o=[...s].length,o!==t))continue;if(a[3]||a[4]){i+=o;continue}if(i-=o,i>0)continue;o=Math.min(o,o+i);let c=[...a[0]][0].length,p=r.slice(0,t+a.index+c+o),m=p.slice(t,-t);return{type:"del",raw:p,text:m,tokens:this.lexer.inlineTokens(m)}}}}autolink(r){let n=this.rules.inline.autolink.exec(r);if(n){let e,a;return n[2]==="@"?(e=n[1],a="mailto:"+e):(e=n[1],a=e),{type:"link",raw:n[0],text:e,href:a,tokens:[{type:"text",raw:e,text:e}]}}}url(r){var e;let n;if(n=this.rules.inline.url.exec(r)){let a,t;if(n[2]==="@")a=n[0],t="mailto:"+a;else{let s;do s=n[0],n[0]=((e=this.rules.inline._backpedal.exec(n[0]))==null?void 0:e[0])??"";while(s!==n[0]);a=n[0],n[1]==="www."?t="http://"+n[0]:t=n[0]}return{type:"link",raw:n[0],text:a,href:t,tokens:[{type:"text",raw:a,text:a}]}}}inlineText(r){let n=this.rules.inline.text.exec(r);if(n){let e=this.lexer.state.inRawBlock;return{type:"text",raw:n[0],text:n[0],escaped:e}}}},R=class z{constructor(n){C(this,"tokens");C(this,"options");C(this,"state");C(this,"inlineQueue");C(this,"tokenizer");this.tokens=[],this.tokens.links=Object.create(null),this.options=n||O,this.options.tokenizer=this.options.tokenizer||new B,this.tokenizer=this.options.tokenizer,this.tokenizer.options=this.options,this.tokenizer.lexer=this,this.inlineQueue=[],this.state={inLink:!1,inRawBlock:!1,top:!0};let e={other:_,block:H.normal,inline:V.normal};this.options.pedantic?(e.block=H.pedantic,e.inline=V.pedantic):this.options.gfm&&(e.block=H.gfm,this.options.breaks?e.inline=V.breaks:e.inline=V.gfm),this.tokenizer.rules=e}static get rules(){return{block:H,inline:V}}static lex(n,e){return new z(e).lex(n)}static lexInline(n,e){return new z(e).inlineTokens(n)}lex(n){n=n.replace(_.carriageReturn,`
`),this.blockTokens(n,this.tokens);for(let e=0;e<this.inlineQueue.length;e++){let a=this.inlineQueue[e];this.inlineTokens(a.src,a.tokens)}return this.inlineQueue=[],this.tokens}blockTokens(n,e=[],a=!1){var t,s,o;for(this.options.pedantic&&(n=n.replace(_.tabCharGlobal,"    ").replace(_.spaceLine,""));n;){let i;if((s=(t=this.options.extensions)==null?void 0:t.block)!=null&&s.some(c=>(i=c.call({lexer:this},n,e))?(n=n.substring(i.raw.length),e.push(i),!0):!1))continue;if(i=this.tokenizer.space(n)){n=n.substring(i.raw.length);let c=e.at(-1);i.raw.length===1&&c!==void 0?c.raw+=`
`:e.push(i);continue}if(i=this.tokenizer.code(n)){n=n.substring(i.raw.length);let c=e.at(-1);(c==null?void 0:c.type)==="paragraph"||(c==null?void 0:c.type)==="text"?(c.raw+=(c.raw.endsWith(`
`)?"":`
`)+i.raw,c.text+=`
`+i.text,this.inlineQueue.at(-1).src=c.text):e.push(i);continue}if(i=this.tokenizer.fences(n)){n=n.substring(i.raw.length),e.push(i);continue}if(i=this.tokenizer.heading(n)){n=n.substring(i.raw.length),e.push(i);continue}if(i=this.tokenizer.hr(n)){n=n.substring(i.raw.length),e.push(i);continue}if(i=this.tokenizer.blockquote(n)){n=n.substring(i.raw.length),e.push(i);continue}if(i=this.tokenizer.list(n)){n=n.substring(i.raw.length),e.push(i);continue}if(i=this.tokenizer.html(n)){n=n.substring(i.raw.length),e.push(i);continue}if(i=this.tokenizer.def(n)){n=n.substring(i.raw.length);let c=e.at(-1);(c==null?void 0:c.type)==="paragraph"||(c==null?void 0:c.type)==="text"?(c.raw+=(c.raw.endsWith(`
`)?"":`
`)+i.raw,c.text+=`
`+i.raw,this.inlineQueue.at(-1).src=c.text):this.tokens.links[i.tag]||(this.tokens.links[i.tag]={href:i.href,title:i.title},e.push(i));continue}if(i=this.tokenizer.table(n)){n=n.substring(i.raw.length),e.push(i);continue}if(i=this.tokenizer.lheading(n)){n=n.substring(i.raw.length),e.push(i);continue}let l=n;if((o=this.options.extensions)!=null&&o.startBlock){let c=1/0,p=n.slice(1),m;this.options.extensions.startBlock.forEach(u=>{m=u.call({lexer:this},p),typeof m=="number"&&m>=0&&(c=Math.min(c,m))}),c<1/0&&c>=0&&(l=n.substring(0,c+1))}if(this.state.top&&(i=this.tokenizer.paragraph(l))){let c=e.at(-1);a&&(c==null?void 0:c.type)==="paragraph"?(c.raw+=(c.raw.endsWith(`
`)?"":`
`)+i.raw,c.text+=`
`+i.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=c.text):e.push(i),a=l.length!==n.length,n=n.substring(i.raw.length);continue}if(i=this.tokenizer.text(n)){n=n.substring(i.raw.length);let c=e.at(-1);(c==null?void 0:c.type)==="text"?(c.raw+=(c.raw.endsWith(`
`)?"":`
`)+i.raw,c.text+=`
`+i.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=c.text):e.push(i);continue}if(n){let c="Infinite loop on byte: "+n.charCodeAt(0);if(this.options.silent){console.error(c);break}else throw new Error(c)}}return this.state.top=!0,e}inline(n,e=[]){return this.inlineQueue.push({src:n,tokens:e}),e}inlineTokens(n,e=[]){var l,c,p,m,u;let a=n,t=null;if(this.tokens.links){let d=Object.keys(this.tokens.links);if(d.length>0)for(;(t=this.tokenizer.rules.inline.reflinkSearch.exec(a))!=null;)d.includes(t[0].slice(t[0].lastIndexOf("[")+1,-1))&&(a=a.slice(0,t.index)+"["+"a".repeat(t[0].length-2)+"]"+a.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(t=this.tokenizer.rules.inline.anyPunctuation.exec(a))!=null;)a=a.slice(0,t.index)+"++"+a.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let s;for(;(t=this.tokenizer.rules.inline.blockSkip.exec(a))!=null;)s=t[2]?t[2].length:0,a=a.slice(0,t.index+s)+"["+"a".repeat(t[0].length-s-2)+"]"+a.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);a=((c=(l=this.options.hooks)==null?void 0:l.emStrongMask)==null?void 0:c.call({lexer:this},a))??a;let o=!1,i="";for(;n;){o||(i=""),o=!1;let d;if((m=(p=this.options.extensions)==null?void 0:p.inline)!=null&&m.some(S=>(d=S.call({lexer:this},n,e))?(n=n.substring(d.raw.length),e.push(d),!0):!1))continue;if(d=this.tokenizer.escape(n)){n=n.substring(d.raw.length),e.push(d);continue}if(d=this.tokenizer.tag(n)){n=n.substring(d.raw.length),e.push(d);continue}if(d=this.tokenizer.link(n)){n=n.substring(d.raw.length),e.push(d);continue}if(d=this.tokenizer.reflink(n,this.tokens.links)){n=n.substring(d.raw.length);let S=e.at(-1);d.type==="text"&&(S==null?void 0:S.type)==="text"?(S.raw+=d.raw,S.text+=d.text):e.push(d);continue}if(d=this.tokenizer.emStrong(n,a,i)){n=n.substring(d.raw.length),e.push(d);continue}if(d=this.tokenizer.codespan(n)){n=n.substring(d.raw.length),e.push(d);continue}if(d=this.tokenizer.br(n)){n=n.substring(d.raw.length),e.push(d);continue}if(d=this.tokenizer.del(n,a,i)){n=n.substring(d.raw.length),e.push(d);continue}if(d=this.tokenizer.autolink(n)){n=n.substring(d.raw.length),e.push(d);continue}if(!this.state.inLink&&(d=this.tokenizer.url(n))){n=n.substring(d.raw.length),e.push(d);continue}let T=n;if((u=this.options.extensions)!=null&&u.startInline){let S=1/0,g=n.slice(1),I;this.options.extensions.startInline.forEach(f=>{I=f.call({lexer:this},g),typeof I=="number"&&I>=0&&(S=Math.min(S,I))}),S<1/0&&S>=0&&(T=n.substring(0,S+1))}if(d=this.tokenizer.inlineText(T)){n=n.substring(d.raw.length),d.raw.slice(-1)!=="_"&&(i=d.raw.slice(-1)),o=!0;let S=e.at(-1);(S==null?void 0:S.type)==="text"?(S.raw+=d.raw,S.text+=d.text):e.push(d);continue}if(n){let S="Infinite loop on byte: "+n.charCodeAt(0);if(this.options.silent){console.error(S);break}else throw new Error(S)}}return e}},F=class{constructor(r){C(this,"options");C(this,"parser");this.options=r||O}space(r){return""}code({text:r,lang:n,escaped:e}){var s;let a=(s=(n||"").match(_.notSpaceStart))==null?void 0:s[0],t=r.replace(_.endingNewline,"")+`
`;return a?'<pre><code class="language-'+P(a)+'">'+(e?t:P(t,!0))+`</code></pre>
`:"<pre><code>"+(e?t:P(t,!0))+`</code></pre>
`}blockquote({tokens:r}){return`<blockquote>
${this.parser.parse(r)}</blockquote>
`}html({text:r}){return r}def(r){return""}heading({tokens:r,depth:n}){return`<h${n}>${this.parser.parseInline(r)}</h${n}>
`}hr(r){return`<hr>
`}list(r){let n=r.ordered,e=r.start,a="";for(let o=0;o<r.items.length;o++){let i=r.items[o];a+=this.listitem(i)}let t=n?"ol":"ul",s=n&&e!==1?' start="'+e+'"':"";return"<"+t+s+`>
`+a+"</"+t+`>
`}listitem(r){return`<li>${this.parser.parse(r.tokens)}</li>
`}checkbox({checked:r}){return"<input "+(r?'checked="" ':"")+'disabled="" type="checkbox"> '}paragraph({tokens:r}){return`<p>${this.parser.parseInline(r)}</p>
`}table(r){let n="",e="";for(let t=0;t<r.header.length;t++)e+=this.tablecell(r.header[t]);n+=this.tablerow({text:e});let a="";for(let t=0;t<r.rows.length;t++){let s=r.rows[t];e="";for(let o=0;o<s.length;o++)e+=this.tablecell(s[o]);a+=this.tablerow({text:e})}return a&&(a=`<tbody>${a}</tbody>`),`<table>
<thead>
`+n+`</thead>
`+a+`</table>
`}tablerow({text:r}){return`<tr>
${r}</tr>
`}tablecell(r){let n=this.parser.parseInline(r.tokens),e=r.header?"th":"td";return(r.align?`<${e} align="${r.align}">`:`<${e}>`)+n+`</${e}>
`}strong({tokens:r}){return`<strong>${this.parser.parseInline(r)}</strong>`}em({tokens:r}){return`<em>${this.parser.parseInline(r)}</em>`}codespan({text:r}){return`<code>${P(r,!0)}</code>`}br(r){return"<br>"}del({tokens:r}){return`<del>${this.parser.parseInline(r)}</del>`}link({href:r,title:n,tokens:e}){let a=this.parser.parseInline(e),t=Sn(r);if(t===null)return a;r=t;let s='<a href="'+r+'"';return n&&(s+=' title="'+P(n)+'"'),s+=">"+a+"</a>",s}image({href:r,title:n,text:e,tokens:a}){a&&(e=this.parser.parseInline(a,this.parser.textRenderer));let t=Sn(r);if(t===null)return P(e);r=t;let s=`<img src="${r}" alt="${P(e)}"`;return n&&(s+=` title="${P(n)}"`),s+=">",s}text(r){return"tokens"in r&&r.tokens?this.parser.parseInline(r.tokens):"escaped"in r&&r.escaped?r.text:P(r.text)}},sn=class{strong({text:r}){return r}em({text:r}){return r}codespan({text:r}){return r}del({text:r}){return r}html({text:r}){return r}text({text:r}){return r}link({text:r}){return""+r}image({text:r}){return""+r}br(){return""}checkbox({raw:r}){return r}},M=class X{constructor(n){C(this,"options");C(this,"renderer");C(this,"textRenderer");this.options=n||O,this.options.renderer=this.options.renderer||new F,this.renderer=this.options.renderer,this.renderer.options=this.options,this.renderer.parser=this,this.textRenderer=new sn}static parse(n,e){return new X(e).parse(n)}static parseInline(n,e){return new X(e).parseInline(n)}parse(n){var a,t;let e="";for(let s=0;s<n.length;s++){let o=n[s];if((t=(a=this.options.extensions)==null?void 0:a.renderers)!=null&&t[o.type]){let l=o,c=this.options.extensions.renderers[l.type].call({parser:this},l);if(c!==!1||!["space","hr","heading","code","table","blockquote","list","html","def","paragraph","text"].includes(l.type)){e+=c||"";continue}}let i=o;switch(i.type){case"space":{e+=this.renderer.space(i);break}case"hr":{e+=this.renderer.hr(i);break}case"heading":{e+=this.renderer.heading(i);break}case"code":{e+=this.renderer.code(i);break}case"table":{e+=this.renderer.table(i);break}case"blockquote":{e+=this.renderer.blockquote(i);break}case"list":{e+=this.renderer.list(i);break}case"checkbox":{e+=this.renderer.checkbox(i);break}case"html":{e+=this.renderer.html(i);break}case"def":{e+=this.renderer.def(i);break}case"paragraph":{e+=this.renderer.paragraph(i);break}case"text":{e+=this.renderer.text(i);break}default:{let l='Token with "'+i.type+'" type was not found.';if(this.options.silent)return console.error(l),"";throw new Error(l)}}}return e}parseInline(n,e=this.renderer){var t,s;let a="";for(let o=0;o<n.length;o++){let i=n[o];if((s=(t=this.options.extensions)==null?void 0:t.renderers)!=null&&s[i.type]){let c=this.options.extensions.renderers[i.type].call({parser:this},i);if(c!==!1||!["escape","html","link","image","strong","em","codespan","br","del","text"].includes(i.type)){a+=c||"";continue}}let l=i;switch(l.type){case"escape":{a+=e.text(l);break}case"html":{a+=e.html(l);break}case"link":{a+=e.link(l);break}case"image":{a+=e.image(l);break}case"checkbox":{a+=e.checkbox(l);break}case"strong":{a+=e.strong(l);break}case"em":{a+=e.em(l);break}case"codespan":{a+=e.codespan(l);break}case"br":{a+=e.br(l);break}case"del":{a+=e.del(l);break}case"text":{a+=e.text(l);break}default:{let c='Token with "'+l.type+'" type was not found.';if(this.options.silent)return console.error(c),"";throw new Error(c)}}}return a}},q,U=(q=class{constructor(r){C(this,"options");C(this,"block");this.options=r||O}preprocess(r){return r}postprocess(r){return r}processAllTokens(r){return r}emStrongMask(r){return r}provideLexer(){return this.block?R.lex:R.lexInline}provideParser(){return this.block?M.parse:M.parseInline}},C(q,"passThroughHooks",new Set(["preprocess","postprocess","processAllTokens","emStrongMask"])),C(q,"passThroughHooksRespectAsync",new Set(["preprocess","postprocess","processAllTokens"])),q),tr=class{constructor(...r){C(this,"defaults",$());C(this,"options",this.setOptions);C(this,"parse",this.parseMarkdown(!0));C(this,"parseInline",this.parseMarkdown(!1));C(this,"Parser",M);C(this,"Renderer",F);C(this,"TextRenderer",sn);C(this,"Lexer",R);C(this,"Tokenizer",B);C(this,"Hooks",U);this.use(...r)}walkTokens(r,n){var a,t;let e=[];for(let s of r)switch(e=e.concat(n.call(this,s)),s.type){case"table":{let o=s;for(let i of o.header)e=e.concat(this.walkTokens(i.tokens,n));for(let i of o.rows)for(let l of i)e=e.concat(this.walkTokens(l.tokens,n));break}case"list":{let o=s;e=e.concat(this.walkTokens(o.items,n));break}default:{let o=s;(t=(a=this.defaults.extensions)==null?void 0:a.childTokens)!=null&&t[o.type]?this.defaults.extensions.childTokens[o.type].forEach(i=>{let l=o[i].flat(1/0);e=e.concat(this.walkTokens(l,n))}):o.tokens&&(e=e.concat(this.walkTokens(o.tokens,n)))}}return e}use(...r){let n=this.defaults.extensions||{renderers:{},childTokens:{}};return r.forEach(e=>{let a={...e};if(a.async=this.defaults.async||a.async||!1,e.extensions&&(e.extensions.forEach(t=>{if(!t.name)throw new Error("extension name required");if("renderer"in t){let s=n.renderers[t.name];s?n.renderers[t.name]=function(...o){let i=t.renderer.apply(this,o);return i===!1&&(i=s.apply(this,o)),i}:n.renderers[t.name]=t.renderer}if("tokenizer"in t){if(!t.level||t.level!=="block"&&t.level!=="inline")throw new Error("extension level must be 'block' or 'inline'");let s=n[t.level];s?s.unshift(t.tokenizer):n[t.level]=[t.tokenizer],t.start&&(t.level==="block"?n.startBlock?n.startBlock.push(t.start):n.startBlock=[t.start]:t.level==="inline"&&(n.startInline?n.startInline.push(t.start):n.startInline=[t.start]))}"childTokens"in t&&t.childTokens&&(n.childTokens[t.name]=t.childTokens)}),a.extensions=n),e.renderer){let t=this.defaults.renderer||new F(this.defaults);for(let s in e.renderer){if(!(s in t))throw new Error(`renderer '${s}' does not exist`);if(["options","parser"].includes(s))continue;let o=s,i=e.renderer[o],l=t[o];t[o]=(...c)=>{let p=i.apply(t,c);return p===!1&&(p=l.apply(t,c)),p||""}}a.renderer=t}if(e.tokenizer){let t=this.defaults.tokenizer||new B(this.defaults);for(let s in e.tokenizer){if(!(s in t))throw new Error(`tokenizer '${s}' does not exist`);if(["options","rules","lexer"].includes(s))continue;let o=s,i=e.tokenizer[o],l=t[o];t[o]=(...c)=>{let p=i.apply(t,c);return p===!1&&(p=l.apply(t,c)),p}}a.tokenizer=t}if(e.hooks){let t=this.defaults.hooks||new U;for(let s in e.hooks){if(!(s in t))throw new Error(`hook '${s}' does not exist`);if(["options","block"].includes(s))continue;let o=s,i=e.hooks[o],l=t[o];U.passThroughHooks.has(s)?t[o]=c=>{if(this.defaults.async&&U.passThroughHooksRespectAsync.has(s))return(async()=>{let m=await i.call(t,c);return l.call(t,m)})();let p=i.call(t,c);return l.call(t,p)}:t[o]=(...c)=>{if(this.defaults.async)return(async()=>{let m=await i.apply(t,c);return m===!1&&(m=await l.apply(t,c)),m})();let p=i.apply(t,c);return p===!1&&(p=l.apply(t,c)),p}}a.hooks=t}if(e.walkTokens){let t=this.defaults.walkTokens,s=e.walkTokens;a.walkTokens=function(o){let i=[];return i.push(s.call(this,o)),t&&(i=i.concat(t.call(this,o))),i}}this.defaults={...this.defaults,...a}}),this}setOptions(r){return this.defaults={...this.defaults,...r},this}lexer(r,n){return R.lex(r,n??this.defaults)}parser(r,n){return M.parse(r,n??this.defaults)}parseMarkdown(r){return(n,e)=>{let a={...e},t={...this.defaults,...a},s=this.onError(!!t.silent,!!t.async);if(this.defaults.async===!0&&a.async===!1)return s(new Error("marked(): The async option was set to true by an extension. Remove async: false from the parse options object to return a Promise."));if(typeof n>"u"||n===null)return s(new Error("marked(): input parameter is undefined or null"));if(typeof n!="string")return s(new Error("marked(): input parameter is of type "+Object.prototype.toString.call(n)+", string expected"));if(t.hooks&&(t.hooks.options=t,t.hooks.block=r),t.async)return(async()=>{let o=t.hooks?await t.hooks.preprocess(n):n,i=await(t.hooks?await t.hooks.provideLexer():r?R.lex:R.lexInline)(o,t),l=t.hooks?await t.hooks.processAllTokens(i):i;t.walkTokens&&await Promise.all(this.walkTokens(l,t.walkTokens));let c=await(t.hooks?await t.hooks.provideParser():r?M.parse:M.parseInline)(l,t);return t.hooks?await t.hooks.postprocess(c):c})().catch(s);try{t.hooks&&(n=t.hooks.preprocess(n));let o=(t.hooks?t.hooks.provideLexer():r?R.lex:R.lexInline)(n,t);t.hooks&&(o=t.hooks.processAllTokens(o)),t.walkTokens&&this.walkTokens(o,t.walkTokens);let i=(t.hooks?t.hooks.provideParser():r?M.parse:M.parseInline)(o,t);return t.hooks&&(i=t.hooks.postprocess(i)),i}catch(o){return s(o)}}}onError(r,n){return e=>{if(e.message+=`
Please report this to https://github.com/markedjs/marked.`,r){let a="<p>An error occurred:</p><pre>"+P(e.message+"",!0)+"</pre>";return n?Promise.resolve(a):a}if(n)return Promise.reject(e);throw e}}},L=new tr;function E(r,n){return L.parse(r,n)}E.options=E.setOptions=function(r){return L.setOptions(r),E.defaults=L.defaults,Tn(E.defaults),E};E.getDefaults=$;E.defaults=O;E.use=function(...r){return L.use(...r),E.defaults=L.defaults,Tn(E.defaults),E};E.walkTokens=function(r,n){return L.walkTokens(r,n)};E.parseInline=L.parseInline;E.Parser=M;E.parser=M.parse;E.Renderer=F;E.TextRenderer=sn;E.Lexer=R;E.lexer=R.lex;E.Tokenizer=B;E.Hooks=U;E.parse=E;E.options;E.setOptions;E.use;E.walkTokens;E.parseInline;M.parse;R.lex;const ar={class:"prd-container"},ir={class:"prd-sidebar"},sr={key:1,style:{padding:"24px","text-align":"center",color:"#999"}},or={class:"prd-content"},cr={class:"doc-title"},lr=["innerHTML"],dr={key:1,class:"text-body"},pr={key:1,class:"empty-state"},ur=vn({__name:"UpdateLog",setup(r){const n=Object.assign({"/PRD/QMS系统菜单重构方案.md":xn,"/PRD/基础数据/供应商档案_详细设计说明书.md":Hn,"/PRD/基础数据/储存地点_详细设计说明书.md":qn,"/PRD/基础数据/客供关系映射_详细设计说明书.md":wn,"/PRD/基础数据/客户档案_详细设计说明书.md":Bn,"/PRD/基础数据/客户特殊要求_详细设计说明书.md":Fn,"/PRD/基础数据/工艺路线_详细设计说明书.md":Yn,"/PRD/基础数据/物料产品档案_详细设计说明书.md":Gn,"/PRD/基础数据/物料分类_详细设计说明书.md":jn,"/PRD/基础数据/生产班组_详细设计说明书.md":Wn,"/PRD/基础数据/计量单位_详细设计说明书.md":Kn,"/PRD/基础数据与质量主数据_需求文档.md":zn,"/PRD/实验室需求文档.md":Xn,"/PRD/执行与控制/IQC来料检验与MRB异常处理详细设计文档.md":$n,"/PRD/执行与控制/实验室检测任务管理详细设计文档.md":Zn,"/PRD/执行与控制/检验任务自动生成与触发详细设计文档.md":Jn,"/PRD/改进与行动/变化点管理/QMS-TDD-变化点管理-v1.0.md":ne,"/PRD/改进与行动/变化点管理/QMS-TDD-变化点管理-v2.0.md":ee,"/PRD/改进与行动/变化点管理/变化点管理详细设计文档-v2.md":re,"/PRD/改进与行动/变化点管理/变化点管理详细设计文档-v3.md":te,"/PRD/改进与行动/变化点管理/变化点管理详细设计文档.md":ae,"/PRD/知识库与培训/受控文档管理_详细设计文档.md":ie,"/PRD/舜富PLM接口文档 .md":se,"/PRD/质量主数据/不良现象与缺陷库_详细设计说明书.md":oe,"/PRD/质量主数据/抽样方案管理_详细设计说明书.md":ce,"/PRD/质量主数据/检验方法管理_详细设计说明书.md":le,"/PRD/质量主数据/检验模板管理_详细设计说明书.md":de,"/PRD/质量主数据/检验计划模型设计方案.md":pe,"/PRD/质量主数据/检验项目管理_详细设计说明书.md":ue,"/PRD/质量主数据/质量检验计划与方案_详细设计说明书.md":me,"/PRD/质量主数据/量检具台账_详细设计说明书.md":Se}),e=on([]),a=on(null),t=l=>{const c=[];for(const[p,m]of Object.entries(l)){const u=p.match(/\/PRD\/(.+)$/);if(!u)continue;const T=u[1].split("/");let S=c,g="";T.forEach((I,f)=>{g+=(g?"/":"")+I;const D=f===T.length-1;let h=S.find(j=>j.title===I);h||(h={title:I,key:g,isLeaf:D,selectable:D},D?h.content=m:h.children=[],S.push(h)),D||(S=h.children)})}return c};kn(()=>{e.value=t(n)});const s=(l,c)=>{const p=c.node.dataRef;p&&p.isLeaf&&(a.value=p)},o=cn(()=>a.value?a.value.title.toLowerCase().endsWith(".md"):!1),i=cn(()=>{var l;return!o.value||!((l=a.value)!=null&&l.content)?"":E.parse(a.value.content)});return(l,c)=>{const p=ln("a-tree"),m=ln("a-empty");return N(),v("div",ar,[b("div",ir,[c[0]||(c[0]=b("div",{class:"sidebar-header"},[b("h3",null,"需求文档 (PRD)")],-1)),e.value.length>0?(N(),bn(p,{key:0,"tree-data":e.value,"default-expand-all":!0,onSelect:s,blockNode:""},null,8,["tree-data"])):(N(),v("div",sr," 未找到文档 "))]),b("div",or,[a.value?(N(),v(Vn,{key:0},[b("h2",cr,dn(a.value.title),1),o.value?(N(),v("div",{key:0,class:"markdown-body",innerHTML:i.value},null,8,lr)):(N(),v("pre",dr,dn(a.value.content),1))],64)):(N(),v("div",pr,[Qn(m,{description:"请在左侧选择要查看的文档"})]))])])}}}),Er=Un(ur,[["__scopeId","data-v-cba065ed"]]);export{Er as default};
